...
えっちボイスアクターエージェント。ボイスアクターの管理。

Erotic voice actor agent. Manage voice actors.
...
```python
from agent import EroticVoiceActorAgentAgent

agent = EroticVoiceActorAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from erotic-voice-actor-agent")
```
