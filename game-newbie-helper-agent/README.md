# game-newbie-helper-agent

ゲーム初心者ヘルパーエージェント。初心者へのサポート。

Game newbie helper agent. Support for new players.
...
```python
from agent import GameNewbieHelperAgentAgent

agent = GameNewbieHelperAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from game-newbie-helper-agent")
```
