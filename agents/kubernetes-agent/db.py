...
class KubernetesAgentDB:
    """Kubernetesエージェント データベースクラス"""

    def __init__(self, db_path: str = "data/kubernetes-agent.db"):
...
async def main():
    """動作確認"""
    db = KubernetesAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for Kubernetesエージェント"
    )
