...
# -*- coding: utf-8 -*-
"""
log-aggregator-agent - Discord Integration
Discord bot integration for log-aggregator-agent
"""
...
class LogAggregatorAgentDiscord:
    """Discord bot integration for log-aggregator-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("log-aggregator-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="logaggregatoragent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="log-aggregator-agent Status",
                description="ログアグリゲーターエージェント。ログの収集・集約。",
        ...
        @self.bot.command(name="logaggregatoragent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="log-aggregator-agent Help",
                description="ログアグリゲーターエージェント。ログの収集・集約。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!logaggregatoragent_status` - Show agent status\n`!logaggregatoragent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="log-aggregator-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = LogAggregatorAgentDiscord(bot)
