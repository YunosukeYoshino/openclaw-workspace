...
ログアグリゲーターエージェント。ログの収集・集約。

## 概要

このエージェントは ログアグリゲーターエージェント。ログの収集・集約。 ためのAIアシスタントです。
...
```python
from agent import LogAggregatorAgent
...
```python
from db import LogAggregatorAgentDB

db = LogAggregatorAgentDB()
...
### LogAggregatorAgent.process_task(task)
...
### LogAggregatorAgentDB.insert_data(data_type, content, metadata)
...
### LogAggregatorAgentDB.query_data(data_type, limit)
