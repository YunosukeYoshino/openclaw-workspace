...
class BaseballFanCommunicationAgentDB:
    """野球ファンコミュニケーションエージェント データベースクラス"""

    def __init__(self, db_path: str = "data/baseball-fan-communication-agent.db"):
...
async def main():
    """動作確認"""
    db = BaseballFanCommunicationAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for 野球ファンコミュニケーションエージェント"
    )
