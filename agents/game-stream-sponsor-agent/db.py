...
class GameStreamSponsorAgentDB:
    """ゲーム配信スポンサーエージェント データベースクラス"""

    def __init__(self, db_path: str = "data/game-stream-sponsor-agent.db"):
...
async def main():
    """動作確認"""
    db = GameStreamSponsorAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for ゲーム配信スポンサーエージェント"
    )
