...
class SecurityAutomationAgentDB:
    """セキュリティ自動化エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/security-automation-agent.db"):
...
async def main():
    """動作確認"""
    db = SecurityAutomationAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for セキュリティ自動化エージェント"
    )
