...
VPNマネージャーエージェント。VPNの管理。

## 概要

このエージェントは VPNマネージャーエージェント。VPNの管理。 ためのAIアシスタントです。
...
```python
from agent import VpnManagerAgent

agent = VpnManagerAgent()
...
```python
from db import VpnManagerAgentDB

db = VpnManagerAgentDB()
...
### VpnManagerAgent.process_task(task)
...
### VpnManagerAgentDB.insert_data(data_type, content, metadata)
...
### VpnManagerAgentDB.query_data(data_type, limit)
