...
# -*- coding: utf-8 -*-
"""
vpn-manager-agent - Discord Integration
Discord bot integration for vpn-manager-agent
"""
...
class VpnManagerAgentDiscord:
    """Discord bot integration for vpn-manager-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("vpn-manager-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="vpnmanageragent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="vpn-manager-agent Status",
                description="VPNマネージャーエージェント。VPNの管理。",
        ...
        @self.bot.command(name="vpnmanageragent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="vpn-manager-agent Help",
                description="VPNマネージャーエージェント。VPNの管理。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!vpnmanageragent_status` - Show agent status\n`!vpnmanageragent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="vpn-manager-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = VpnManagerAgentDiscord(bot)
