# game-lobby-manager-agent

ゲームロビーマネージャーエージェント。ロビーの管理。

## 概要

このエージェントは ゲームロビーマネージャーエージェント。ロビーの管理。 ためのAIアシスタントです。
...
```python
from agent import GameLobbyManagerAgent

agent = GameLobbyManagerAgent()
...
```python
from db import GameLobbyManagerAgentDB
...
### GameLobbyManagerAgent.process_task(task)
...
### GameLobbyManagerAgentDB.insert_data(data_type, content, metadata)
...
### GameLobbyManagerAgentDB.query_data(data_type, limit)
