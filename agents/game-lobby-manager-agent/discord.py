...
# -*- coding: utf-8 -*-
"""
game-lobby-manager-agent - Discord Integration
Discord bot integration for game-lobby-manager-agent
"""
...
class GameLobbyManagerAgentDiscord:
    """Discord bot integration for game-lobby-manager-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("game-lobby-manager-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="gamelobbymanageragent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="game-lobby-manager-agent Status",
        ...
        @self.bot.command(name="gamelobbymanageragent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="game-lobby-manager-agent Help",
                description="ゲームロビーマネージャーエージェント。ロビーの管理。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!gamelobbymanageragent_status` - Show agent status\n`!gamelobbymanageragent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="game-lobby-manager-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = GameLobbyManagerAgentDiscord(bot)
