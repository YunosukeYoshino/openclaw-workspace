#!/usr/bin/env python3
"""
deploy-scheduler-agent - Discord Botモジュール
"""

import discord
from discord.ext import commands
from db import DeploySchedulerAgentDB

class DeploySchedulerAgentDiscordBot(commands.Bot):
    """deploy-scheduler-agent Discord Bot"""

    def __init__(self, db_path: str = "deploy-scheduler-agent.db"):
        intents = discord.Intents.default()
        intents.message_content = True
        super().__init__(command_prefix="!", intents=intents)
        self.db = DeploySchedulerAgentDB(db_path)
...
def main():
    """メイン関数"""
    bot = DeploySchedulerAgentDiscordBot()
