...
class GameCrossPlatformAgentDB:
    """ゲームクロスプラットフォームエージェント データベースクラス"""

    def __init__(self, db_path: str = "data/game-cross-platform-agent.db"):
...
async def main():
    """動作確認"""
    db = GameCrossPlatformAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for ゲームクロスプラットフォームエージェント"
    )
