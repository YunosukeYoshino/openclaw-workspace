#!/usr/bin/env python3
"""
野球ライブ配信スケジュールエージェント - Database Module
...
class Baseball_Live_Schedule_AgentDB:
    """野球ライブ配信スケジュールエージェント Database"""

    def __init__(self, db_path: Optional[Path] = None):
        if db_path is None:
            db_path = Path(__file__).parent / "data" / "baseball-live-schedule-agent.db"
...
def main():
    """メイン関数"""
    db = Baseball_Live_Schedule_AgentDB()
    stats = db.get_stats()
    print("📊 データベース統計:")
