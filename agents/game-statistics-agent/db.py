...
class GameStatisticsAgentDB:
    """ゲーム統計エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/game-statistics-agent.db"):
...
async def main():
    """動作確認"""
    db = GameStatisticsAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for ゲーム統計エージェント"
    )
