...
class BaseballInsightsAgentDB:
    """野球インサイト分析エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/baseball-insights-agent.db"):
...
async def main():
    """動作確認"""
    db = BaseballInsightsAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for 野球インサイト分析エージェント"
    )
