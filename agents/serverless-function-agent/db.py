...
class ServerlessFunctionAgentDB:
    """サーバーレス関数エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/serverless-function-agent.db"):
...
async def main():
    """動作確認"""
    db = ServerlessFunctionAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for サーバーレス関数エージェント"
    )
