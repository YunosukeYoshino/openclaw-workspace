#!/usr/bin/env python3
"""
野球海外エージェント データベースモジュール
Baseball Overseas Agent Database Module
...
class BaseballOverseasAgentDB:
    """野球海外エージェント データベースクラス"""
    ...
    def init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            # インデックス作成
            cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_overseas_leagues_status ON overseas_leagues(status)")
            cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_overseas_leagues_created ON overseas_leagues(created_at)")
    ...
    def insert_entry(self, title: str, description: str = "", data_json: str = "{}", status: str = "active") -> int:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                f"""
                INSERT INTO overseas_leagues (title, description, data_json, status)
                VALUES (?, ?, ?, ?)
                """,
    ...
    def get_entry(self, entry_id: int) -> Optional[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"SELECT * FROM overseas_leagues WHERE id = ?",
                (entry_id,)
            )
            row = cursor.fetchone()
    ...
    def list_entries(self, limit: int = 100, offset: int = 0, status: str = None) -> List[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            ...
            query = f"SELECT * FROM overseas_leagues"
            params = []
    ...
    def update_entry(self, entry_id: int, **kwargs) -> bool:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                f"UPDATE overseas_leagues SET {', '.join(update_fields)} WHERE id = ?",
                params
            )
            conn.commit()
    ...
    def delete_entry(self, entry_id: int) -> bool:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"DELETE FROM overseas_leagues WHERE id = ?",
                (entry_id,)
            )
            conn.commit()
            return cursor.rowcount > 0
