#!/usr/bin/env python3
"""
Discord Bot Integration for ゲームコンテンツカレンダーエージェント / Game Content Calendar Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for game-content-calendar-agent"""
    ...
    def __init__(self, command_prefix: str = "!", db=None):
        ...
        self.db = db
        self.agent_id = "game-content-calendar-agent"

    async def setup_hook(self):
        """Bot setup"""
        logger.info(f"Setting up {self.agent_id} Discord bot...")
        await self.add_cog(GameContentCalendarAgentCommands(self))
...
class GameContentCalendarAgentCommands(commands.Cog):
    """Commands for game-content-calendar-agent"""
...
"""
        help_text = help_text.replace("game-content-calendar-agent", agent['id'])
        help_text = help_text.replace("ゲームコンテンツカレンダーエージェント", agent['name_ja'])
        help_text = help_text.replace("            - Event Aggregation
            - Schedule Management
            - Reminders
            - Collab Suggestions
            - Timing Optimization", features_list)
        help_text = help_text.replace("GameContentCalendarAgent", snake_to_camel(agent['id']))
