...
このエージェントは 野球栄養エージェント。栄養管理・食事プランの提供。 ためのAIアシスタントです。
...
```python
from agent import BaseballNutritionAgent
...
```python
from db import BaseballNutritionAgentDB
...
### BaseballNutritionAgent.process_task(task)
...
### BaseballNutritionAgentDB.insert_data(data_type, content, metadata)
...
### BaseballNutritionAgentDB.query_data(data_type, limit)
