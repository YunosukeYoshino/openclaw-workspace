...
# -*- coding: utf-8 -*-
"""
erotic-crm-agent - Discord Integration
Discord bot integration for erotic-crm-agent
"""
...
class EroticCrmAgentDiscord:
    """Discord bot integration for erotic-crm-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("erotic-crm-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="eroticcrmagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="erotic-crm-agent Status",
                description="えっちCRMエージェント。顧客関係管理。",
        ...
        @self.bot.command(name="eroticcrmagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="erotic-crm-agent Help",
                description="えっちCRMエージェント。顧客関係管理。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!eroticcrmagent_status` - Show agent status\n`!eroticcrmagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="erotic-crm-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = EroticCrmAgentDiscord(bot)
