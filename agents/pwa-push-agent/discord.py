...
class Pwa_push_agentDiscordBot(commands.Bot):
    # pwa-push-agent Discord ボット
    ...
    async def setup_hook(self):
        # ボット起動時の設定
        await self.add_cog(Pwa_push_agentCommands(self))
...
class Pwa_push_agentCommands(commands.Cog):
    # pwa-push-agent コマンド
    ...
    @commands.command(name="pwa_push_agent")
    async def pwa_push_agent(self, ctx: commands.Context, action: str = "list", *, args: str = ""):
        # メインコマンド
        if action == "list":
            ...
            if not entries:
                ...
                return
            embed = discord.Embed(title="Pwa Push Agent 一覧", color=discord.Color.blue())
    ...
    @commands.command(name="pwa_push_agent_status")
    async def pwa_push_agent_status(self, ctx: commands.Context):
        # ステータス確認
        entries = self.bot.db.list_entries(status="active")
        embed = discord.Embed(title="Pwa Push Agent ステータス", color=discord.Color.gold())
        embed.add_field(name="アクティブエントリー", value=str(len(entries)))
        await ctx.send(embed=embed)

    @commands.command(name="pwa_push_agent_delete")
    async def pwa_push_agent_delete(self, ctx: commands.Context, entry_id: int):
        # エントリー削除
