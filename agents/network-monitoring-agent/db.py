...
class NetworkMonitoringAgentDB:
    """ネットワーク監視エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/network-monitoring-agent.db"):
...
async def main():
    """動作確認"""
    db = NetworkMonitoringAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for ネットワーク監視エージェント"
    )
