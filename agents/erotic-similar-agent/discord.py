#!/usr/bin/env python3
"""
Discord Bot module for erotic-similar-agent
類似えっちコンテンツ検索エージェントのDiscord Botモジュール
"""
...
from .agent import EroticSimilarAgentAgent
from .db import get_db


class DiscordBot(commands.Bot):
    """類似えっちコンテンツ検索エージェント Discord Bot"""

    def __init__(self, agent: EroticSimilarAgentAgent):
...
def create_bot(agent: EroticSimilarAgentAgent, token: str) -> DiscordBot:
    """Botインスタンスを作成"""
    ...
    async def list_entries(ctx, limit: int = 10):
        ...
        embed = discord.Embed(title="類似えっちコンテンツ検索エージェント", color=discord.Color.blue())
...
async def run_bot(agent: EroticSimilarAgentAgent, token: str):
    """Botを実行"""
