...
# -*- coding: utf-8 -*-
"""
flutter-dev-agent - Discord Integration
Discord bot integration for flutter-dev-agent
"""
...
class FlutterDevAgentDiscord:
    """Discord bot integration for flutter-dev-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("flutter-dev-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="flutterdevagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="flutter-dev-agent Status",
                description="Flutter開発エージェント。Flutter開発。",
        ...
        @self.bot.command(name="flutterdevagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="flutter-dev-agent Help",
                description="Flutter開発エージェント。Flutter開発。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!flutterdevagent_status` - Show agent status\n`!flutterdevagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="flutter-dev-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = FlutterDevAgentDiscord(bot)
