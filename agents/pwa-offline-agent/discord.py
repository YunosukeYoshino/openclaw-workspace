...
class Pwa_offline_agentDiscordBot(commands.Bot):
    # pwa-offline-agent Discord ボット
    ...
    async def setup_hook(self):
        # ボット起動時の設定
        await self.add_cog(Pwa_offline_agentCommands(self))
...
class Pwa_offline_agentCommands(commands.Cog):
    # pwa-offline-agent コマンド

    def __init__(self, bot: commands.Bot):
        # 初期化
        self.bot = bot

    @commands.command(name="pwa_offline_agent")
    async def pwa_offline_agent(self, ctx: commands.Context, action: str = "list", *, args: str = ""):
        # メインコマンド
        if action == "list":
            ...
            if not entries:
                ...
                return
            embed = discord.Embed(title="Pwa Offline Agent 一覧", color=discord.Color.blue())
    ...
    @commands.command(name="pwa_offline_agent_status")
    async def pwa_offline_agent_status(self, ctx: commands.Context):
        # ステータス確認
        entries = self.bot.db.list_entries(status="active")
        embed = discord.Embed(title="Pwa Offline Agent ステータス", color=discord.Color.gold())
        embed.add_field(name="アクティブエントリー", value=str(len(entries)))
        await ctx.send(embed=embed)

    @commands.command(name="pwa_offline_agent_delete")
    async def pwa_offline_agent_delete(self, ctx: commands.Context, entry_id: int):
        # エントリー削除
