...
class BaseballStatsVizAgentDB:
    """野球統計可視化エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/baseball-stats-viz-agent.db"):
...
async def main():
    """動作確認"""
    db = BaseballStatsVizAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for 野球統計可視化エージェント"
    )
