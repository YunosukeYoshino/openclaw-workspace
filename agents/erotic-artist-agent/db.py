#!/usr/bin/env python3
"""
えっちなイラストレーター管理エージェント - データベースモジュール
...
class EroticArtistAgentDB:
    """えっちなイラストレーター管理エージェント データベースクラス"""

    def __init__(self, db_path: Optional[str] = None):
        """初期化"""
        self.db_path = Path(db_path) if db_path else Path(__file__).parent / "erotic-artist-agent.db"
