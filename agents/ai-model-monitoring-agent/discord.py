#!/usr/bin/env python3
"""
Discord bot integration for ai-model-monitoring-agent
"""
...
from agent import AiModelMonitoringAgent
...
class DiscordBot(commands.Bot):
    """Discord bot for ai-model-monitoring-agent"""
    ...
    def __init__(self):
        ...
        self.agent = AiModelMonitoringAgent()
    ...
    async def setup_hook(self):
        ...
        logger.info("Commands synced")
            ...
            bot.tree.command(name="model_status")(self.model_status)
            bot.tree.command(name="metrics_history")(self.metrics_history)
            bot.tree.command(name="alerts")(self.alerts)
            bot.tree.command(name="acknowledge_alert")(self.acknowledge_alert)

    async def model_status(self, interaction):
        """Handle model_status command"""
        await interaction.response.send_message(f"{agent_name}: model_status command received!")

    async def metrics_history(self, interaction):
        """Handle metrics_history command"""
        await interaction.response.send_message(f"{agent_name}: metrics_history command received!")
