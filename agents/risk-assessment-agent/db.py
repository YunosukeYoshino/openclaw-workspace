...
class RiskAssessmentAgentDB:
    """リスクアセスメントエージェント データベースクラス"""

    def __init__(self, db_path: str = "data/risk-assessment-agent.db"):
...
async def main():
    """動作確認"""
    db = RiskAssessmentAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for リスクアセスメントエージェント"
    )
