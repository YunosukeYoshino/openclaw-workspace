#!/usr/bin/env python3
"""
ゲーム大会エージェント データベースモジュール
Game Tournament Agent Database Module
...
class GameTournamentAgentDB:
    """ゲーム大会エージェント データベースクラス"""
    ...
    def init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            # インデックス作成
            cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_tournaments_status ON tournaments(status)")
            cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_tournaments_created ON tournaments(created_at)")
    ...
    def insert_entry(self, title: str, description: str = "", data_json: str = "{}", status: str = "active") -> int:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                f"""
                INSERT INTO tournaments (title, description, data_json, status)
                VALUES (?, ?, ?, ?)
                """,
    ...
    def get_entry(self, entry_id: int) -> Optional[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"SELECT * FROM tournaments WHERE id = ?",
                (entry_id,)
            )
            row = cursor.fetchone()
    ...
    def delete_entry(self, entry_id: int) -> bool:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"DELETE FROM tournaments WHERE id = ?",
                (entry_id,)
            )
            conn.commit()
            return cursor.rowcount > 0
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(f"SELECT COUNT(*) FROM tournaments WHERE status = 'active'")
            active_entries = cursor.fetchone()[0]

            cursor.execute(f"SELECT COUNT(*) FROM participants")
    ...
    def insert_item(self, name: str, data_json: str = "{}") -> int:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                f"""
                INSERT INTO participants (name, data_json)
                VALUES (?, ?)
                """,
                (name, data_json)
            )
    ...
    def get_items(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"SELECT * FROM participants ORDER BY created_at DESC LIMIT ?",
                (limit,)
            )
            rows = cursor.fetchall()
            return [dict(row) for row in rows]
