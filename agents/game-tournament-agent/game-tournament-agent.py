...
class GameTournamentAgent:
    ...
    def add_entry(self, title: str, description: str, data_json: str = "{}") -> int:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO tournaments (title, description, data_json) VALUES (?, ?, ?)",
    ...
    def list_entries(self, limit: int = 100) -> List[Dict]:
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM tournaments ORDER BY created_at DESC LIMIT ?", (limit,))
    ...
    def delete_entry(self, entry_id: int) -> bool:
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute(f"DELETE FROM tournaments WHERE id = ?", (entry_id,))
        conn.commit()
        conn.close()
    ...
    def search_entries(self, query: str, limit: int = 50) -> List[Dict]:
        ...
        cursor = conn.cursor()
        cursor.execute(
            f"SELECT * FROM tournaments WHERE title LIKE ? OR description LIKE ? ORDER BY created_at DESC LIMIT ?",
...
def main():
    ...
    print("ゲーム大会エージェント initialized!")
    print("Database:", agent.db_path)

    # サンプルデータ追加
    entry_id = agent.add_entry(
        title="サンプルエントリー",
        description="ゲーム大会エージェントのサンプルデータ"
    )
