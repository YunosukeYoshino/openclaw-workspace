...
# -*- coding: utf-8 -*-
"""
ios-dev-agent - Discord Integration
Discord bot integration for ios-dev-agent
"""
...
class IosDevAgentDiscord:
    """Discord bot integration for ios-dev-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("ios-dev-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="iosdevagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="ios-dev-agent Status",
                description="iOS開発エージェント。iOSアプリの開発。",
        ...
        @self.bot.command(name="iosdevagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="ios-dev-agent Help",
                description="iOS開発エージェント。iOSアプリの開発。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!iosdevagent_status` - Show agent status\n`!iosdevagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="ios-dev-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = IosDevAgentDiscord(bot)
