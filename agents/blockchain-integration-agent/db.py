...
class BlockchainIntegrationAgentDB:
    """ブロックチェーン統合エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/blockchain-integration-agent.db"):
...
async def main():
    """動作確認"""
    db = BlockchainIntegrationAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for ブロックチェーン統合エージェント"
    )
