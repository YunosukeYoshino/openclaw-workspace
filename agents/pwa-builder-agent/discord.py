...
class Pwa_builder_agentDiscordBot(commands.Bot):
    # pwa-builder-agent Discord ボット
    ...
    async def setup_hook(self):
        # ボット起動時の設定
        await self.add_cog(Pwa_builder_agentCommands(self))
...
class Pwa_builder_agentCommands(commands.Cog):
    # pwa-builder-agent コマンド

    def __init__(self, bot: commands.Bot):
        # 初期化
        self.bot = bot

    @commands.command(name="pwa_builder_agent")
    async def pwa_builder_agent(self, ctx: commands.Context, action: str = "list", *, args: str = ""):
        # メインコマンド
        if action == "list":
            ...
            if not entries:
                ...
                return
            embed = discord.Embed(title="Pwa Builder Agent 一覧", color=discord.Color.blue())
    ...
    @commands.command(name="pwa_builder_agent_status")
    async def pwa_builder_agent_status(self, ctx: commands.Context):
        # ステータス確認
        entries = self.bot.db.list_entries(status="active")
        embed = discord.Embed(title="Pwa Builder Agent ステータス", color=discord.Color.gold())
        embed.add_field(name="アクティブエントリー", value=str(len(entries)))
        await ctx.send(embed=embed)

    @commands.command(name="pwa_builder_agent_delete")
    async def pwa_builder_agent_delete(self, ctx: commands.Context, entry_id: int):
        # エントリー削除
