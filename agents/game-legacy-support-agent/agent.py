...
class GameLegacySupportAgent:
    """ゲームレガシーサポートエージェント"""

    def __init__(self, config: Optional[Dict[str, Any]] = None):
        self.config = config or {}
        self.name = "game-legacy-support-agent"
        self.title = "ゲームレガシーサポートエージェント"
        self.description = "ゲームレガシーシステムのサポート"
...
async def main():
    """メイン処理"""
    agent = GameLegacySupportAgent()
    await agent.initialize()

    sample_task = {
        "id": "sample_001",
        "data": {
            "message": "Sample task for ゲームレガシーサポートエージェント"
        }
    }
