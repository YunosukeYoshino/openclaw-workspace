...
class HealthWellnessAgent:
    ...
    def init_db(self):
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS health_metrics (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, content TEXT NOT NULL, priority INTEGER DEFAULT 0, status TEXT DEFAULT 'active', metadata TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
        ...
        # Create indices
        cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_{self._sanitize_table("health_metrics")}_status ON health_metrics(status)')
        cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_{self._sanitize_table("health_metrics")}_priority ON health_metrics(priority)')
    ...
    def add_item(self, title, content, priority=0, metadata=None):
        ...
        cursor.execute("INSERT INTO health_metrics (title, content, priority, metadata) VALUES (?, ?, ?, ?)", (title, content, priority, metadata_json))
        self.conn.commit()
    ...
    def update_item(self, item_id, **kwargs):
        ...
        if updates:
            query = f"UPDATE health_metrics SET {', '.join(updates)} WHERE id = ?"
    ...
    def delete_item(self, item_id):
        cursor = self.conn.cursor()
        cursor.execute("DELETE FROM health_metrics WHERE id = ?", (item_id,))
        self.conn.commit()
...
def main():
    ...
    try:
        print(f"Health & Wellness Agent initialized")
        print(f"Database: {agent.db_path}")
        print("Available commands: track, analyze, recommend, goal, report")
    finally:
        agent.close()
