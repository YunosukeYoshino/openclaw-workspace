...
ゲームAR UXエージェント。AR体験のUX設計・管理。

## 概要

このエージェントは ゲームAR UXエージェント。AR体験のUX設計・管理。 ためのAIアシスタントです。
...
```python
from agent import GameArUxAgent
...
```python
from db import GameArUxAgentDB
...
### GameArUxAgent.process_task(task)
...
### GameArUxAgentDB.insert_data(data_type, content, metadata)
...
### GameArUxAgentDB.query_data(data_type, limit)
