...
class GameReinforcementAgentDB:
    """ゲーム強化エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/game-reinforcement-agent.db"):
...
async def main():
    """動作確認"""
    db = GameReinforcementAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for ゲーム強化エージェント"
    )
