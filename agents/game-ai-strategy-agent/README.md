# ゲームAI戦略エージェント / ゲームAI戦略エージェント (EN)

AIによるゲーム戦略分析・提案エージェント / AIによるゲーム戦略分析・提案エージェント

## Overview / 概要

AIを活用したAIによるゲーム戦略分析・提案エージェント。
...
```python
from agent import GameAiStrategyAgent

agent = GameAiStrategyAgent()
...
- `!add_ai-strategy <title> <content>` - Add entry / エントリー追加
- `!list_ai-strategy [limit]` - List entries / エントリー一覧
- `!get_ai-strategy <id>` - Get entry / エントリー取得
- `!search_ai-strategy <query>` - Search entries / エントリー検索
- `!stats_ai-strategy` - Get statistics / 統計情報
