#!/usr/bin/env python3
"""
Discord Bot Integration for 野球戦術・ルール進化エージェント / Baseball Evolution Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for baseball-evolution-agent"""
    ...
    def __init__(self, command_prefix: str = "!", db=None):
        ...
        self.db = db
        self.agent_id = "baseball-evolution-agent"

    async def setup_hook(self):
        """Bot setup"""
        logger.info(f"Setting up {self.agent_id} Discord bot...")
        await self.add_cog(BaseballEvolutionAgentCommands(self))
...
class BaseballEvolutionAgentCommands(commands.Cog):
    """Commands for baseball-evolution-agent"""
...
"""
        help_text = help_text.replace("baseball-evolution-agent", agent['id'])
        help_text = help_text.replace("野球戦術・ルール進化エージェント", agent['name_ja'])
        help_text = help_text.replace("            - Tactic Evolution Tracking
            - Rule Change Analysis
            - Era Comparison
            - Future Prediction
            - Trend Analysis
            - Historical Search", features_list)
        help_text = help_text.replace("BaseballEvolutionAgent", snake_to_camel(agent['id']))
