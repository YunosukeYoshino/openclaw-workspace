...
このエージェントは ネットワークセキュリティエージェント。ネットワークセキュリティの管理。 ためのAIアシスタントです。
...
```python
from agent import NetworkSecurityAgent
...
```python
from db import NetworkSecurityAgentDB
...
### NetworkSecurityAgent.process_task(task)
...
### NetworkSecurityAgentDB.insert_data(data_type, content, metadata)
...
### NetworkSecurityAgentDB.query_data(data_type, limit)
