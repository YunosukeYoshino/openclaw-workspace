...
# -*- coding: utf-8 -*-
"""
network-security-agent - Database Module
SQLite database management for network-security-agent
"""
...
class NetworkSecurityAgentDB:
    """Database manager for network-security-agent"""

    def __init__(self, db_path: str = None):
        if db_path is None:
            db_path = str(Path(__file__).parent / "network-security-agent.db")
...
if __name__ == "__main__":
    db = NetworkSecurityAgentDB()
    print("Database for network-security-agent initialized at " + str(db.db_path))
