...
```python
from erotic-platform-agent.db import EroticPlatformAgentDB

# データベース初期化 / Initialize database
db = EroticPlatformAgentDB()
db.init_database()
...
```python
from erotic-platform-agent.discord import EroticPlatformAgentBot

# Bot起動 / Start bot
bot = EroticPlatformAgentBot(command_prefix="!")
...
|---------------------|---------------------|
| `!platformagentadd <title> [description]` | 新しいエントリーを追加 / Add new entry |
| `!platformagentlist [limit]` | エントリー一覧を表示 / List entries |
| `!platformagentget <id>` | エントリーの詳細を表示 / Get entry details |
| `!platformagentsearch <query>` | エントリーを検索 / Search entries |
| `!platformagentstats` | 統計情報を表示 / Show statistics |
| `!platformagenthelp` | ヘルプを表示 / Show help |
