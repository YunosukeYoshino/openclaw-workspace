...
このエージェントは データ可視化エージェント。データ可視化の管理・生成。 ためのAIアシスタントです。
...
```python
from agent import DataVisualizationAgent
...
```python
from db import DataVisualizationAgentDB
...
### DataVisualizationAgent.process_task(task)
...
### DataVisualizationAgentDB.insert_data(data_type, content, metadata)
...
### DataVisualizationAgentDB.query_data(data_type, limit)
