#!/usr/bin/env python3
"""
野球メディカルエージェント データベースモジュール
Baseball Medical Agent Database Module
...
class BaseballMedicalAgentDB:
    """野球メディカルエージェント データベースクラス"""

    def __init__(self, db_path: str = "baseball-medical-agent.db"):
    ...
    def init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            # インデックス作成
            cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_injuries_status ON injuries(status)")
            cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_injuries_created ON injuries(created_at)")
    ...
    def insert_entry(self, title: str, description: str = "", data_json: str = "{}", status: str = "active") -> int:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                f"""
                INSERT INTO injuries (title, description, data_json, status)
                VALUES (?, ?, ?, ?)
                """,
    ...
    def get_entry(self, entry_id: int) -> Optional[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"SELECT * FROM injuries WHERE id = ?",
                (entry_id,)
            )
            row = cursor.fetchone()
    ...
    def update_entry(self, entry_id: int, **kwargs) -> bool:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                f"UPDATE injuries SET {', '.join(update_fields)} WHERE id = ?",
                params
            )
            conn.commit()
    ...
    def delete_entry(self, entry_id: int) -> bool:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"DELETE FROM injuries WHERE id = ?",
                (entry_id,)
            )
            conn.commit()
            return cursor.rowcount > 0
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(f"SELECT COUNT(*) FROM injuries")
            total_entries = cursor.fetchone()[0]

            cursor.execute(f"SELECT COUNT(*) FROM injuries WHERE status = 'active'")
