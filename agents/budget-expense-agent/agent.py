...
class BudgetExpenseAgent:
    """BudgetExpenseAgent - budget expense agentは、budgetexpenseの管理と追跡を行うAIエージェントです。"""

    def __init__(self, db_path: str = None):
        """初期化"""
        if db_path is None:
            db_path = Path(__file__).parent / "budget-expense-agent.db"
        self.db = Database(str(db_path))
        self.table_name = "budget_expense_agent"
...
if __name__ == "__main__":
    agent = BudgetExpenseAgent()
    print(f"{BudgetExpenseAgent} initialized")
