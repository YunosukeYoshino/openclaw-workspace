...
class MessageAgent:
    """MessageAgent - メッセージとコミュニケーションの管理を担当するAIエージェント。テキストメッセージ、通信ログ、コンタクト情報を追跡します。"""

    def __init__(self, db_path: str = None):
        """初期化"""
        if db_path is None:
            db_path = Path(__file__).parent / "message-agent.db"
        self.db = Database(str(db_path))
        self.table_name = "message_agent"
...
if __name__ == "__main__":
    agent = MessageAgent()
    print(f"{MessageAgent} initialized")
