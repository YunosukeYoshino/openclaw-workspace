#!/usr/bin/env python3
"""
Database module for セキュリティインシデントレスポンスエージェント
"""
...
class Database:
    """Database handler for セキュリティインシデントレスポンスエージェント"""

    def __init__(self, db_path: Optional[str] = None):
        if db_path is None:
            db_path = Path(__file__).parent / "security-incident-response-agent.db"
