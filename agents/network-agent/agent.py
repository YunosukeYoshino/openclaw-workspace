...
class NetworkAgent:
    """NetworkAgent - Network management Discord bot with NLP-based message processing."""

    def __init__(self, db_path: str = None):
        """初期化"""
        if db_path is None:
            db_path = Path(__file__).parent / "network-agent.db"
        self.db = Database(str(db_path))
        self.table_name = "network_agent"
...
if __name__ == "__main__":
    agent = NetworkAgent()
    print(f"{NetworkAgent} initialized")
