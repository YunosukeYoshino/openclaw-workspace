...
class Abac_agentDiscordBot(commands.Bot):
    # abac-agent Discord ボット
    ...
    async def setup_hook(self):
        # ボット起動時の設定
        await self.add_cog(Abac_agentCommands(self))
...
class Abac_agentCommands(commands.Cog):
    # abac-agent コマンド
    ...
    @commands.command(name="abac_agent")
    async def abac_agent(self, ctx: commands.Context, action: str = "list", *, args: str = ""):
        # メインコマンド
        if action == "list":
            ...
            if not entries:
                ...
                return
            embed = discord.Embed(title="Abac Agent 一覧", color=discord.Color.blue())
    ...
    @commands.command(name="abac_agent_status")
    async def abac_agent_status(self, ctx: commands.Context):
        # ステータス確認
        entries = self.bot.db.list_entries(status="active")
        embed = discord.Embed(title="Abac Agent ステータス", color=discord.Color.gold())
        embed.add_field(name="アクティブエントリー", value=str(len(entries)))
        await ctx.send(embed=embed)

    @commands.command(name="abac_agent_delete")
    async def abac_agent_delete(self, ctx: commands.Context, entry_id: int):
        # エントリー削除
