...
class BaseballAmenitiesAgentDB:
    """野球場アメニティエージェント データベースクラス"""

    def __init__(self, db_path: str = "data/baseball-amenities-agent.db"):
...
async def main():
    """動作確認"""
    db = BaseballAmenitiesAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for 野球場アメニティエージェント"
    )
