...
class IncidentPostMortemAgentDB:
    """インシデント事後分析エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/incident-post-mortem-agent.db"):
...
async def main():
    """動作確認"""
    db = IncidentPostMortemAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for インシデント事後分析エージェント"
    )
