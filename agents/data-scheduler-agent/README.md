...
データスケジューラーエージェント。データ処理のスケジューリング。

## 概要

このエージェントは データスケジューラーエージェント。データ処理のスケジューリング。 ためのAIアシスタントです。
...
```python
from agent import DataSchedulerAgent

agent = DataSchedulerAgent()
...
```python
from db import DataSchedulerAgentDB
...
### DataSchedulerAgent.process_task(task)
...
### DataSchedulerAgentDB.insert_data(data_type, content, metadata)
...
### DataSchedulerAgentDB.query_data(data_type, limit)
