...
このエージェントは Lambdaマネージャーエージェント。AWS Lambdaの管理。 ためのAIアシスタントです。
...
```python
from agent import LambdaManagerAgent
...
```python
from db import LambdaManagerAgentDB
...
### LambdaManagerAgent.process_task(task)
...
### LambdaManagerAgentDB.insert_data(data_type, content, metadata)
...
### LambdaManagerAgentDB.query_data(data_type, limit)
