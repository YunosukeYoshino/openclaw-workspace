...
class TokenizationAgentDB:
    """トークン化エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/tokenization-agent.db"):
...
async def main():
    """動作確認"""
    db = TokenizationAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for トークン化エージェント"
    )
