...
class GameProgressionAgentDB:
    """ゲーム進行管理エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/game-progression-agent.db"):
...
async def main():
    """動作確認"""
    db = GameProgressionAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for ゲーム進行管理エージェント"
    )
