#!/usr/bin/env python3
"""
Discord Bot Integration for ゲーム攻略ビルダーエージェント / Game Walkthrough Builder Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for game-walkthrough-builder-agent"""
    ...
    def __init__(self, command_prefix: str = "!", db=None):
        ...
        self.db = db
        self.agent_id = "game-walkthrough-builder-agent"

    async def setup_hook(self):
        """Bot setup"""
        logger.info(f"Setting up {self.agent_id} Discord bot...")
        await self.add_cog(GameWalkthroughBuilderAgentCommands(self))
...
class GameWalkthroughBuilderAgentCommands(commands.Cog):
    """Commands for game-walkthrough-builder-agent"""
...
"""
        help_text = help_text.replace("game-walkthrough-builder-agent", agent['id'])
        help_text = help_text.replace("ゲーム攻略ビルダーエージェント", agent['name_ja'])
        help_text = help_text.replace("            - Play Log Analysis
            - Article Generation
            - Media Integration
            - Guide Structuring
            - Navigation System", features_list)
        help_text = help_text.replace("GameWalkthroughBuilderAgent", snake_to_camel(agent['id']))
