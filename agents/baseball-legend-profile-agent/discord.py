#!/usr/bin/env python3
"""
Discord Bot Integration for 野球伝説選手プロフィールエージェント / Baseball Legend Profile Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for baseball-legend-profile-agent"""
    ...
    def __init__(self, command_prefix: str = "!", db=None):
        ...
        self.db = db
        self.agent_id = "baseball-legend-profile-agent"

    async def setup_hook(self):
        """Bot setup"""
        logger.info(f"Setting up {self.agent_id} Discord bot...")
        await self.add_cog(BaseballLegendProfileAgentCommands(self))
...
class BaseballLegendProfileAgentCommands(commands.Cog):
    """Commands for baseball-legend-profile-agent"""
...
"""
        help_text = help_text.replace("baseball-legend-profile-agent", agent['id'])
        help_text = help_text.replace("野球伝説選手プロフィールエージェント", agent['name_ja'])
        help_text = help_text.replace("            - Legend Profiles
            - Statistics Tracking
            - Highlight Collection
            - Cross-Gen Comparison
            - Influence Metrics
            - Search & Discovery", features_list)
        help_text = help_text.replace("BaseballLegendProfileAgent", snake_to_camel(agent['id']))
