...
えっちなシリーズ作品、連載状況、シリーズ順を管理するエージェント
...
```python
from erotic-series-agent.db import EroticSeriesAgentDB

# データベース初期化 / Initialize database
db = EroticSeriesAgentDB()
db.init_database()
...
```python
from erotic-series-agent.discord import EroticSeriesAgentBot

# Bot起動 / Start bot
bot = EroticSeriesAgentBot(command_prefix="!")
...
|---------------------|---------------------|
| `!seriesagentadd <title> [description]` | 新しいエントリーを追加 / Add new entry |
| `!seriesagentlist [limit]` | エントリー一覧を表示 / List entries |
| `!seriesagentget <id>` | エントリーの詳細を表示 / Get entry details |
| `!seriesagentsearch <query>` | エントリーを検索 / Search entries |
| `!seriesagentstats` | 統計情報を表示 / Show statistics |
| `!seriesagenthelp` | ヘルプを表示 / Show help |
