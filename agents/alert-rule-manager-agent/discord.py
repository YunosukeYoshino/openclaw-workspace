#!/usr/bin/env python3
"""
alert-rule-manager-agent - Discord Botモジュール
"""

import discord
from discord.ext import commands
from db import AlertRuleManagerAgentDB

class AlertRuleManagerAgentDiscordBot(commands.Bot):
    """alert-rule-manager-agent Discord Bot"""

    def __init__(self, db_path: str = "alert-rule-manager-agent.db"):
        intents = discord.Intents.default()
        intents.message_content = True
        super().__init__(command_prefix="!", intents=intents)
        self.db = AlertRuleManagerAgentDB(db_path)
...
def main():
    """メイン関数"""
    bot = AlertRuleManagerAgentDiscordBot()
