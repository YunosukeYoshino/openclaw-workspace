...
class Mfa_agentDiscordBot(commands.Bot):
    # mfa-agent Discord ボット
    ...
    async def setup_hook(self):
        # ボット起動時の設定
        await self.add_cog(Mfa_agentCommands(self))
...
class Mfa_agentCommands(commands.Cog):
    # mfa-agent コマンド
    ...
    @commands.command(name="mfa_agent")
    async def mfa_agent(self, ctx: commands.Context, action: str = "list", *, args: str = ""):
        # メインコマンド
        if action == "list":
            ...
            if not entries:
                ...
                return
            embed = discord.Embed(title="Mfa Agent 一覧", color=discord.Color.blue())
    ...
    @commands.command(name="mfa_agent_status")
    async def mfa_agent_status(self, ctx: commands.Context):
        # ステータス確認
        entries = self.bot.db.list_entries(status="active")
        embed = discord.Embed(title="Mfa Agent ステータス", color=discord.Color.gold())
        embed.add_field(name="アクティブエントリー", value=str(len(entries)))
        await ctx.send(embed=embed)

    @commands.command(name="mfa_agent_delete")
    async def mfa_agent_delete(self, ctx: commands.Context, entry_id: int):
        # エントリー削除
