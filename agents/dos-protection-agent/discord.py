...
# -*- coding: utf-8 -*-
"""
dos-protection-agent - Discord Integration
Discord bot integration for dos-protection-agent
"""
...
class DosProtectionAgentDiscord:
    """Discord bot integration for dos-protection-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("dos-protection-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="dosprotectionagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="dos-protection-agent Status",
                description="DoS保護エージェント。DoS攻撃の検知・防御。",
        ...
        @self.bot.command(name="dosprotectionagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="dos-protection-agent Help",
                description="DoS保護エージェント。DoS攻撃の検知・防御。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!dosprotectionagent_status` - Show agent status\n`!dosprotectionagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="dos-protection-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = DosProtectionAgentDiscord(bot)
