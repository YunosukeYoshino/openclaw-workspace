...
class IncidentManagementAgentDB:
    """インシデント管理エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/incident-management-agent.db"):
...
async def main():
    """動作確認"""
    db = IncidentManagementAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for インシデント管理エージェント"
    )
