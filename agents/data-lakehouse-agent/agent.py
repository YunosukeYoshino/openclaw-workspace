...
class DataLakehouseAgent:
    """データレイクハウスエージェント。データレイクハウスアーキテクチャの管理。"""

    def __init__(self, db_path=None):
        """Initialize the agent"""
        from .db import data_lakehouse_agentDatabase

        self.db_path = db_path or Path(f"/workspace/agents/data-lakehouse-agent/data.db")
        self.db = data_lakehouse_agentDatabase(self.db_path)
        self.commands = ["create_table", "zones", "table_lineage", "query_lakehouse"]
    ...
    async def handle_command(self, cmd: str, args: list, user_id: str = None):
        ...
        if cmd == "create_table" and len(commands) > 0:
            return await self.create_table(args, user_id)
    ...
    async def create_table(self, args: list, user_id: str = None):
        """Handle create_table command"""
        logger.info(f"create_table: {args}")
