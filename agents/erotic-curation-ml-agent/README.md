...
このエージェントは えっちキュレーションMLエージェント。MLによるコンテンツキュレーション。 ためのAIアシスタントです。
...
```python
from agent import EroticCurationMlAgent
...
```python
from db import EroticCurationMlAgentDB
...
### EroticCurationMlAgent.process_task(task)
...
### EroticCurationMlAgentDB.insert_data(data_type, content, metadata)
...
### EroticCurationMlAgentDB.query_data(data_type, limit)
