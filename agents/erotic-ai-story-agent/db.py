...
class EroticAiStoryAgentDB:
    """えっちAIストーリーエージェント データベースクラス"""

    def __init__(self, db_path: str = "erotic-ai-story-agent.db"):
        """初期化
    ...
    def get_stories(self, limit: int = 100) -> List[Dict]:
        """storiesリスト取得
        ...
        """
        return self.execute_query(
            f"SELECT * FROM stories ORDER BY created_at DESC LIMIT ?",
            (limit,)
        )

    def add_stories(self, **kwargs) -> int:
        """stories追加

        Returns:
            追加したID
        """
        # stories_テーブルにデータを追加するロジック
        # 各エージェントの要件に合わせて実装
        pass
