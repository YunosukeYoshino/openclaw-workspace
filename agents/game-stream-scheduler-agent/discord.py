#!/usr/bin/env python3
"""
game-stream-scheduler-agent - Discord Botモジュール
"""

import discord
from discord.ext import commands
from db import GameStreamSchedulerAgentDB

class GameStreamSchedulerAgentDiscordBot(commands.Bot):
    """game-stream-scheduler-agent Discord Bot"""

    def __init__(self, db_path: str = "game-stream-scheduler-agent.db"):
        intents = discord.Intents.default()
        intents.message_content = True
        super().__init__(command_prefix="!", intents=intents)
        self.db = GameStreamSchedulerAgentDB(db_path)
...
def main():
    """メイン関数"""
    bot = GameStreamSchedulerAgentDiscordBot()
