...
# -*- coding: utf-8 -*-
"""
android-dev-agent - Discord Integration
Discord bot integration for android-dev-agent
"""
...
class AndroidDevAgentDiscord:
    """Discord bot integration for android-dev-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("android-dev-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="androiddevagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="android-dev-agent Status",
                description="Android開発エージェント。Androidアプリの開発。",
        ...
        @self.bot.command(name="androiddevagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="android-dev-agent Help",
                description="Android開発エージェント。Androidアプリの開発。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!androiddevagent_status` - Show agent status\n`!androiddevagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="android-dev-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = AndroidDevAgentDiscord(bot)
