...
Android開発エージェント。Androidアプリの開発。

## 概要

このエージェントは Android開発エージェント。Androidアプリの開発。 ためのAIアシスタントです。
...
```python
from agent import AndroidDevAgent

agent = AndroidDevAgent()
...
```python
from db import AndroidDevAgentDB

db = AndroidDevAgentDB()
...
### AndroidDevAgent.process_task(task)
...
### AndroidDevAgentDB.insert_data(data_type, content, metadata)
...
### AndroidDevAgentDB.query_data(data_type, limit)
