...
class ApiVersioningAgentDB:
    """APIバージョニングエージェント データベースクラス"""

    def __init__(self, db_path: str = "data/api-versioning-agent.db"):
...
async def main():
    """動作確認"""
    db = ApiVersioningAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for APIバージョニングエージェント"
    )
