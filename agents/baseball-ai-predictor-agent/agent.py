...
class BaseballAiPredictorAgent:
    """Baseball AI Predictor Agent"""
    ...
    def init_db(self):
        ...
        # Create indices
        cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_{self._sanitize_table("baseball_predictions")}_status ON baseball_predictions(status)')
        cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_{self._sanitize_table("baseball_predictions")}_confidence ON baseball_predictions(confidence)')
    ...
    def add_item(self, title, content, ai_features=None, confidence=None, source=None, category=None):
        ...
        cursor.execute("INSERT INTO baseball_predictions (title, content, ai_features, confidence, source, category) VALUES (?, ?, ?, ?, ?, ?)", (title, content, ai_features_json, confidence, source, category))
        self.conn.commit()
        return cursor.lastrowid

    def get_items(self, status=None, min_confidence=None, limit=None):
        cursor = self.conn.cursor()
        query = "SELECT * FROM baseball_predictions"
        params = []
...
def main():
    agent = BaseballAiPredictorAgent()
    try:
        print(f"Baseball AI Predictor Agent initialized")
        print(f"Database: {agent.db_path}")
        print("Available commands: predict, analyze, train, evaluate, history")
    finally:
        agent.close()
