#!/usr/bin/env python3
"""
Discord Bot Integration for ゲームカウンターピックエージェント / Game Counter Pick Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for game-counter-pick-agent"""
    ...
    async def setup_hook(self):
        ...
        await self.add_cog(GameCounterPickAgentCommands(self))
...
class GameCounterPickAgentCommands(commands.Cog):
    """Commands for game-counter-pick-agent"""
...
"""
        help_text = help_text.replace("game-counter-pick-agent", agent['id'])
        help_text = help_text.replace("ゲームカウンターピックエージェント", agent['name_ja'])
        help_text = help_text.replace("            - Counter Pick Suggest
            - Synergy Analysis
            - Anti-Synergy Detection
            - Build Guide Gen
            - Strategy Explain
            - Win Rate Stats", features_list)
        help_text = help_text.replace("GameCounterPickAgent", snake_to_camel(agent['id']))
