...
このエージェントは SAML統合エージェント。SAMLシングルサインオンの統合。 ためのAIアシスタントです。
...
```python
from agent import SamlIntegrationAgent

agent = SamlIntegrationAgent()
...
```python
from db import SamlIntegrationAgentDB

db = SamlIntegrationAgentDB()
...
### SamlIntegrationAgent.process_task(task)
...
### SamlIntegrationAgentDB.insert_data(data_type, content, metadata)
...
### SamlIntegrationAgentDB.query_data(data_type, limit)
