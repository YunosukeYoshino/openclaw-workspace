...
This agent helps manage baseball-history-agent.
...
```python
from agent import BaseballHistoryAgent

agent = BaseballHistoryAgent()
stats = agent.get_all()
print(stats)
```
...
# 野球歴史記録管理エージェント（詳細野球統計分析エージェント）

野球の歴史や記録を管理するエージェント

## 概要

このエージェントはbaseball-history-agentを管理するのに役立ちます。
