...
class BaseballHistoryAgentDB:
    """野球歴史エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/baseball-history-agent.db"):
...
async def main():
    """動作確認"""
    db = BaseballHistoryAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for 野球歴史エージェント"
    )
