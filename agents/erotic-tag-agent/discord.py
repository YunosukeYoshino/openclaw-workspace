#!/usr/bin/env python3
"""
えっちなコンテンツのタグ・検索管理エージェント - Discord Botモジュール
...
class EroticTagAgentBot(commands.Bot):
    """えっちなコンテンツのタグ・検索管理エージェント Discord Bot"""
    ...
    def __init__(self, db_path: str = None, command_prefix: str = "!"):
        ...
        self.db = EroticTagAgentDB(db_path)
        self.db.initialize()
...
def get_bot(db_path: str = None, command_prefix: str = "!"):
    ...
    global bot
    if bot is None:
        bot = EroticTagAgentBot(db_path, command_prefix)
        ...
        async def show_help(ctx: commands.Context):
            ...
            embed = discord.Embed(
                ...
                description="えっちなコンテンツのタグ・検索管理エージェントの使い方",
                color=0x00aaff
            )
