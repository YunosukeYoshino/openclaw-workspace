#!/usr/bin/env python3
"""
Discord Bot Integration for ゲームクリップ編集エージェント / Game Clip Editor Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for game-clip-editor-agent"""
    ...
    async def setup_hook(self):
        ...
        await self.add_cog(GameClipEditorAgentCommands(self))
...
class GameClipEditorAgentCommands(commands.Cog):
    """Commands for game-clip-editor-agent"""
...
"""
        help_text = help_text.replace("game-clip-editor-agent", agent['id'])
        help_text = help_text.replace("ゲームクリップ編集エージェント", agent['name_ja'])
        help_text = help_text.replace("            - Highlight Detection
            - Clip Extraction
            - Text Overlay
            - Effects & Music
            - SNS Optimization
            - Export Management", features_list)
        help_text = help_text.replace("GameClipEditorAgent", snake_to_camel(agent['id']))
