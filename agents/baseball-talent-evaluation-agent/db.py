...
class BaseballTalentEvaluationAgentDB:
    """野球タレント評価エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/baseball-talent-evaluation-agent.db"):
...
async def main():
    """動作確認"""
    db = BaseballTalentEvaluationAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for 野球タレント評価エージェント"
    )
