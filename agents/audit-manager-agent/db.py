...
class AuditManagerAgentDB:
    """監査マネージャーエージェント データベースクラス"""

    def __init__(self, db_path: str = "data/audit-manager-agent.db"):
...
async def main():
    """動作確認"""
    db = AuditManagerAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for 監査マネージャーエージェント"
    )
