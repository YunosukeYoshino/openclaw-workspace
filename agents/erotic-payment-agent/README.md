# erotic-payment-agent

えっち決済エージェント。決済処理の管理。

## 概要

このエージェントは えっち決済エージェント。決済処理の管理。 ためのAIアシスタントです。
...
```python
from agent import EroticPaymentAgent

agent = EroticPaymentAgent()
...
```python
from db import EroticPaymentAgentDB

db = EroticPaymentAgentDB()
...
### EroticPaymentAgent.process_task(task)
...
### EroticPaymentAgentDB.insert_data(data_type, content, metadata)
...
### EroticPaymentAgentDB.query_data(data_type, limit)
