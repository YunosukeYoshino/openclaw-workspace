...
# -*- coding: utf-8 -*-
"""
erotic-payment-agent - Discord Integration
Discord bot integration for erotic-payment-agent
"""
...
class EroticPaymentAgentDiscord:
    """Discord bot integration for erotic-payment-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("erotic-payment-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="eroticpaymentagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="erotic-payment-agent Status",
                description="えっち決済エージェント。決済処理の管理。",
        ...
        @self.bot.command(name="eroticpaymentagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="erotic-payment-agent Help",
                description="えっち決済エージェント。決済処理の管理。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!eroticpaymentagent_status` - Show agent status\n`!eroticpaymentagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="erotic-payment-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = EroticPaymentAgentDiscord(bot)
