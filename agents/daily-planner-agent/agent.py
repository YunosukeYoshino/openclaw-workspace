...
class DailyPlannerAgent:
    ...
    def init_db(self):
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS daily_plans (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, content TEXT NOT NULL, priority INTEGER DEFAULT 0, status TEXT DEFAULT 'active', metadata TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
        ...
        # Create indices
        cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_{self._sanitize_table("daily_plans")}_status ON daily_plans(status)')
        cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_{self._sanitize_table("daily_plans")}_priority ON daily_plans(priority)')
    ...
    def add_item(self, title, content, priority=0, metadata=None):
        ...
        cursor.execute("INSERT INTO daily_plans (title, content, priority, metadata) VALUES (?, ?, ?, ?)", (title, content, priority, metadata_json))
        self.conn.commit()
    ...
    def update_item(self, item_id, **kwargs):
        ...
        if updates:
            query = f"UPDATE daily_plans SET {', '.join(updates)} WHERE id = ?"
    ...
    def delete_item(self, item_id):
        cursor = self.conn.cursor()
        cursor.execute("DELETE FROM daily_plans WHERE id = ?", (item_id,))
        self.conn.commit()
...
def main():
    ...
    try:
        print(f"Daily Planner Agent initialized")
        print(f"Database: {agent.db_path}")
        print("Available commands: plan, schedule, task, reminder, optimize")
    finally:
        agent.close()
