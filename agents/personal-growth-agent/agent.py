...
class PersonalGrowthAgent:
    ...
    def init_db(self):
        ...
        # Create habits table
        cursor.execute("CREATE TABLE IF NOT EXISTS habits (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, content TEXT NOT NULL, priority INTEGER DEFAULT 0, status TEXT DEFAULT 'active', metadata TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
        ...
        # Create indices
        cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_{self._sanitize_table("habits")}_status ON habits(status)')
        cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_{self._sanitize_table("habits")}_priority ON habits(priority)')
    ...
    def add_item(self, title, content, priority=0, metadata=None):
        ...
        cursor.execute("INSERT INTO habits (title, content, priority, metadata) VALUES (?, ?, ?, ?)", (title, content, priority, metadata_json))
        self.conn.commit()
...
def main():
    agent = PersonalGrowthAgent()
    try:
        print(f"Personal Growth Agent initialized")
        print(f"Database: {agent.db_path}")
        print("Available commands: habit, goal, progress, reflect, motivate")
    finally:
        agent.close()
