...
脅威検知エージェント。セキュリティ脅威の検知。

## 概要

このエージェントは 脅威検知エージェント。セキュリティ脅威の検知。 ためのAIアシスタントです。
...
```python
from db import ThreatDetectionAgentDB
...
### ThreatDetectionAgent.process_task(task)
...
### ThreatDetectionAgentDB.insert_data(data_type, content, metadata)
...
### ThreatDetectionAgentDB.query_data(data_type, limit)
