...
class GameImmersiveAgent:
    """ゲームイマーシブエージェント。没入型体験の設計・実装。"""

    def __init__(self, db_path=None):
        """Initialize agent"""
        from .db import game_immersive_agentDatabase

        self.db_path = db_path or Path(f"/workspace/agents/game-immersive-agent/data.db")
        self.db = game_immersive_agentDatabase(self.db_path)
        self.commands = ["create_experience", "add_haptics", "configure_audio", "immersion_test"]
    ...
    async def handle_command(self, cmd: str, args: list, user_id: str = None):
        ...
        if cmd == "create_experience" and len(commands) > 0:
            return await self.create_experience(args, user_id)
    ...
    async def create_experience(self, args: list, user_id: str = None):
        """Handle create_experience command"""
        logger.info(f"create_experience: {args}")
