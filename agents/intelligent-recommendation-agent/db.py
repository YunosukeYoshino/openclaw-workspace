#!/usr/bin/env python3
"""
Database module for Intelligent Recommendation Agent
インテリジェント推薦エージェント データベースモジュール
"""
...
class IntelligentRecommendationAgentDB:
    """Database handler for Intelligent Recommendation Agent"""
    ...
    def _init_tables(self) -> None:
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS recommendations (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, content TEXT NOT NULL, source TEXT, category TEXT, status TEXT DEFAULT 'active', metadata TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
        ...
        cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_{self._sanitize_table("recommendations")}_category ON recommendations(category)')
