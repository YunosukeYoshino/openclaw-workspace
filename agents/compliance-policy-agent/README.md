...
コンプライアンスポリシーエージェント。コンプライアンスポリシーの管理。

## 概要

このエージェントは コンプライアンスポリシーエージェント。コンプライアンスポリシーの管理。 ためのAIアシスタントです。
...
```python
from agent import CompliancePolicyAgent
...
```python
from db import CompliancePolicyAgentDB

db = CompliancePolicyAgentDB()
...
### CompliancePolicyAgent.process_task(task)
...
### CompliancePolicyAgentDB.insert_data(data_type, content, metadata)
...
### CompliancePolicyAgentDB.query_data(data_type, limit)
