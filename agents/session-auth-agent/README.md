...
セッション認証エージェント。セッション管理・認証。

## 概要

このエージェントは セッション認証エージェント。セッション管理・認証。 ためのAIアシスタントです。
...
```python
from agent import SessionAuthAgent
...
```python
from db import SessionAuthAgentDB

db = SessionAuthAgentDB()
...
### SessionAuthAgent.process_task(task)
...
### SessionAuthAgentDB.insert_data(data_type, content, metadata)
...
### SessionAuthAgentDB.query_data(data_type, limit)
