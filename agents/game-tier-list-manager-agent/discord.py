#!/usr/bin/env python3
"""
Discord Bot Integration for ゲームTierリスト管理エージェント / Game Tier List Manager Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for game-tier-list-manager-agent"""
    ...
    def __init__(self, command_prefix: str = "!", db=None):
        ...
        self.db = db
        self.agent_id = "game-tier-list-manager-agent"

    async def setup_hook(self):
        """Bot setup"""
        logger.info(f"Setting up {self.agent_id} Discord bot...")
        await self.add_cog(GameTierListManagerAgentCommands(self))
...
class GameTierListManagerAgentCommands(commands.Cog):
    """Commands for game-tier-list-manager-agent"""
...
"""
        help_text = help_text.replace("game-tier-list-manager-agent", agent['id'])
        help_text = help_text.replace("ゲームTierリスト管理エージェント", agent['name_ja'])
        help_text = help_text.replace("            - Tier List Creation
            - Community Voting
            - Version Management
            - Role-Based Lists
            - History Tracking
            - Trend Visualization", features_list)
        help_text = help_text.replace("GameTierListManagerAgent", snake_to_camel(agent['id']))
