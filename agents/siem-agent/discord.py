...
# -*- coding: utf-8 -*-
"""
siem-agent - Discord Integration
Discord bot integration for siem-agent
"""
...
class SiemAgentDiscord:
    """Discord bot integration for siem-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("siem-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="siemagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="siem-agent Status",
                description="SIEMエージェント。SIEM（Security Information and Event Management）の管理。",
        ...
        @self.bot.command(name="siemagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="siem-agent Help",
                description="SIEMエージェント。SIEM（Security Information and Event Management）の管理。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!siemagent_status` - Show agent status\n`!siemagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="siem-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = SiemAgentDiscord(bot)
