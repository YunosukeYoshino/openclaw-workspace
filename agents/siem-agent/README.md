...
SIEMエージェント。SIEM（Security Information and Event Management）の管理。

## 概要

このエージェントは SIEMエージェント。SIEM（Security Information and Event Management）の管理。 ためのAIアシスタントです。
...
### SiemAgentDB.insert_data(data_type, content, metadata)
...
### SiemAgentDB.query_data(data_type, limit)
