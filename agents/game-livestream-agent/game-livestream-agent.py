...
class GameLivestreamAgent:
    ...
    def add_entry(self, title: str, description: str, data_json: str = "{}") -> int:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO livestreams (title, description, data_json) VALUES (?, ?, ?)",
    ...
    def list_entries(self, limit: int = 100) -> List[Dict]:
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM livestreams ORDER BY created_at DESC LIMIT ?", (limit,))
    ...
    def update_entry(self, entry_id: int, title: str = None, description: str = None, data_json: str = None) -> bool:
        ...
        cursor.execute(
            f"UPDATE livestreams SET {', '.join(updates)} WHERE id = ?",
            params
        )
        conn.commit()
        conn.close()
    ...
    def delete_entry(self, entry_id: int) -> bool:
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute(f"DELETE FROM livestreams WHERE id = ?", (entry_id,))
        conn.commit()
        conn.close()
    ...
    def search_entries(self, query: str, limit: int = 50) -> List[Dict]:
        ...
        cursor = conn.cursor()
        cursor.execute(
            f"SELECT * FROM livestreams WHERE title LIKE ? OR description LIKE ? ORDER BY created_at DESC LIMIT ?",
...
def main():
    ...
    print("ゲームライブ配信エージェント initialized!")
    print("Database:", agent.db_path)

    # サンプルデータ追加
    entry_id = agent.add_entry(
        title="サンプルエントリー",
        description="ゲームライブ配信エージェントのサンプルデータ"
    )
