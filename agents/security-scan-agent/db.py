...
class SecurityScanAgentDB:
    """セキュリティスキャンエージェント データベースクラス"""

    def __init__(self, db_path: str = "data/security-scan-agent.db"):
...
async def main():
    """動作確認"""
    db = SecurityScanAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for セキュリティスキャンエージェント"
    )
