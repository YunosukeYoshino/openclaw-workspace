...
class Rbac_agentDiscordBot(commands.Bot):
    # rbac-agent Discord ボット
    ...
    async def setup_hook(self):
        # ボット起動時の設定
        await self.add_cog(Rbac_agentCommands(self))
...
class Rbac_agentCommands(commands.Cog):
    # rbac-agent コマンド
    ...
    @commands.command(name="rbac_agent")
    async def rbac_agent(self, ctx: commands.Context, action: str = "list", *, args: str = ""):
        # メインコマンド
        if action == "list":
            ...
            if not entries:
                ...
                return
            embed = discord.Embed(title="Rbac Agent 一覧", color=discord.Color.blue())
    ...
    @commands.command(name="rbac_agent_status")
    async def rbac_agent_status(self, ctx: commands.Context):
        # ステータス確認
        entries = self.bot.db.list_entries(status="active")
        embed = discord.Embed(title="Rbac Agent ステータス", color=discord.Color.gold())
        embed.add_field(name="アクティブエントリー", value=str(len(entries)))
        await ctx.send(embed=embed)

    @commands.command(name="rbac_agent_delete")
    async def rbac_agent_delete(self, ctx: commands.Context, entry_id: int):
        # エントリー削除
