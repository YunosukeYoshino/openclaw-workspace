#!/usr/bin/env python3
"""
Database module for ゲームストリームクリエイターツールエージェント
"""
...
class Database:
    """Database handler for ゲームストリームクリエイターツールエージェント"""

    def __init__(self, db_path: Optional[str] = None):
        if db_path is None:
            db_path = Path(__file__).parent / "game-stream-creator-tools-agent.db"
