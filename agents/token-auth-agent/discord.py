...
# -*- coding: utf-8 -*-
"""
token-auth-agent - Discord Integration
Discord bot integration for token-auth-agent
"""
...
class TokenAuthAgentDiscord:
    """Discord bot integration for token-auth-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("token-auth-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="tokenauthagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="token-auth-agent Status",
                description="トークン認証エージェント。トークンベース認証の管理。",
        ...
        @self.bot.command(name="tokenauthagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="token-auth-agent Help",
                description="トークン認証エージェント。トークンベース認証の管理。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!tokenauthagent_status` - Show agent status\n`!tokenauthagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="token-auth-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = TokenAuthAgentDiscord(bot)
