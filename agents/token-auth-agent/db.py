...
# -*- coding: utf-8 -*-
"""
token-auth-agent - Database Module
SQLite database management for token-auth-agent
"""
...
class TokenAuthAgentDB:
    """Database manager for token-auth-agent"""

    def __init__(self, db_path: str = None):
        if db_path is None:
            db_path = str(Path(__file__).parent / "token-auth-agent.db")
...
if __name__ == "__main__":
    db = TokenAuthAgentDB()
    print("Database for token-auth-agent initialized at " + str(db.db_path))
