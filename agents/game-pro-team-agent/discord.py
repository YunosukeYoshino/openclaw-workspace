#!/usr/bin/env python3
"""
Discord Bot Integration for ゲームプロチームエージェント / Game Pro Team Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for game-pro-team-agent"""
    ...
    def __init__(self, command_prefix: str = "!", db=None):
        ...
        self.db = db
        self.agent_id = "game-pro-team-agent"

    async def setup_hook(self):
        """Bot setup"""
        logger.info(f"Setting up {self.agent_id} Discord bot...")
        await self.add_cog(GameProTeamAgentCommands(self))
...
class GameProTeamAgentCommands(commands.Cog):
    """Commands for game-pro-team-agent"""
...
"""
        help_text = help_text.replace("game-pro-team-agent", agent['id'])
        help_text = help_text.replace("ゲームプロチームエージェント", agent['name_ja'])
        help_text = help_text.replace("            - Team Profiles
            - Roster Tracking
            - Performance Stats
            - Strategy Analysis
            - Transfer News
            - Contract Info", features_list)
        help_text = help_text.replace("GameProTeamAgent", snake_to_camel(agent['id']))
