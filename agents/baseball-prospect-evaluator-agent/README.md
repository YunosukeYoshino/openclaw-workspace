...
このエージェントは 野球プロスペクト評価エージェント。有望選手の評価・分析。 ためのAIアシスタントです。
...
```python
from agent import BaseballProspectEvaluatorAgent
...
```python
from db import BaseballProspectEvaluatorAgentDB
...
### BaseballProspectEvaluatorAgent.process_task(task)
...
### BaseballProspectEvaluatorAgentDB.insert_data(data_type, content, metadata)
...
### BaseballProspectEvaluatorAgentDB.query_data(data_type, limit)
