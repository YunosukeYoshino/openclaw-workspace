...
class EdgeInferenceAgentDB:
    """エッジ推論エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/edge-inference-agent.db"):
...
async def main():
    """動作確認"""
    db = EdgeInferenceAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for エッジ推論エージェント"
    )
