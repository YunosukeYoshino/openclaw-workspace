...
class EroticInsightsAgentDB:
    """えっちインサイトエージェント データベースクラス"""

    def __init__(self, db_path: str = "data/erotic-insights-agent.db"):
...
async def main():
    """動作確認"""
    db = EroticInsightsAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for えっちインサイトエージェント"
    )
