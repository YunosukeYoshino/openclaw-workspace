#!/usr/bin/env python3
"""
Discord Bot Integration for ゲームビルド最適化エージェント / Game Build Optimizer Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for game-build-optimizer-agent"""
    ...
    async def setup_hook(self):
        ...
        await self.add_cog(GameBuildOptimizerAgentCommands(self))
...
class GameBuildOptimizerAgentCommands(commands.Cog):
    """Commands for game-build-optimizer-agent"""
...
"""
        help_text = help_text.replace("game-build-optimizer-agent", agent['id'])
        help_text = help_text.replace("ゲームビルド最適化エージェント", agent['name_ja'])
        help_text = help_text.replace("            - Build Optimization
            - Item Suggestions
            - Skill Priorities
            - Rune Configs
            - Patch Updates
            - Build Sharing", features_list)
        help_text = help_text.replace("GameBuildOptimizerAgent", snake_to_camel(agent['id']))
