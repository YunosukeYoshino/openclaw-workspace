...
class EroticBehaviorAgentDB:
    """えっち行動分析エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/erotic-behavior-agent.db"):
...
async def main():
    """動作確認"""
    db = EroticBehaviorAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for えっち行動分析エージェント"
    )
