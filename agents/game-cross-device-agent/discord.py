#!/usr/bin/env python3
"""
game-cross-device-agent - Discord Botモジュール
"""

import discord
from discord.ext import commands
from db import GameCrossDeviceAgentDB

class GameCrossDeviceAgentDiscordBot(commands.Bot):
    """game-cross-device-agent Discord Bot"""

    def __init__(self, db_path: str = "game-cross-device-agent.db"):
        intents = discord.Intents.default()
        intents.message_content = True
        super().__init__(command_prefix="!", intents=intents)
        self.db = GameCrossDeviceAgentDB(db_path)
...
def main():
    """メイン関数"""
    bot = GameCrossDeviceAgentDiscordBot()
