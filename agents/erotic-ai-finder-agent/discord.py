#!/usr/bin/env python3
"""
Discord Bot Integration for えっちAI検索エージェント / Erotic AI Finder Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for erotic-ai-finder-agent"""
    ...
    def __init__(self, command_prefix: str = "!", db=None):
        ...
        self.db = db
        self.agent_id = "erotic-ai-finder-agent"

    async def setup_hook(self):
        """Bot setup"""
        logger.info(f"Setting up {self.agent_id} Discord bot...")
        await self.add_cog(EroticAiFinderAgentCommands(self))
...
class EroticAiFinderAgentCommands(commands.Cog):
    """Commands for erotic-ai-finder-agent"""
...
"""
        help_text = help_text.replace("erotic-ai-finder-agent", agent['id'])
        help_text = help_text.replace("えっちAI検索エージェント", agent['name_ja'])
        help_text = help_text.replace("            - Natural Search
            - Semantic Search
            - Image Search
            - Video Search
            - Complex Filters
            - Personalized Rank", features_list)
        help_text = help_text.replace("EroticAiFinderAgent", snake_to_camel(agent['id']))
