#!/usr/bin/env python3
"""
time-series-db-agent - Discord Botモジュール
"""

import discord
from discord.ext import commands
from db import TimeSeriesDbAgentDB

class TimeSeriesDbAgentDiscordBot(commands.Bot):
    """time-series-db-agent Discord Bot"""

    def __init__(self, db_path: str = "time-series-db-agent.db"):
        intents = discord.Intents.default()
        intents.message_content = True
        super().__init__(command_prefix="!", intents=intents)
        self.db = TimeSeriesDbAgentDB(db_path)
...
def main():
    """メイン関数"""
    bot = TimeSeriesDbAgentDiscordBot()
