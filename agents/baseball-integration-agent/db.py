#!/usr/bin/env python3
"""
Database module for Baseball Integration Agent
野球統合エージェント データベースモジュール
"""
...
class BaseballIntegrationAgentDB:
    """Database handler for Baseball Integration Agent"""
    ...
    def _init_tables(self) -> None:
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS baseball_integrations (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, content TEXT NOT NULL, source TEXT, category TEXT, status TEXT DEFAULT 'active', metadata TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
        ...
        cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_{self._sanitize_table("baseball_integrations")}_category ON baseball_integrations(category)')
