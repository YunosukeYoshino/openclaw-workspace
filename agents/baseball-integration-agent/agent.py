...
class BaseballIntegrationAgent:
    """Baseball Integration Agent"""
    ...
    def init_db(self):
        ...
        # Create baseball_integrations table
        cursor.execute(f"CREATE TABLE IF NOT EXISTS baseball_integrations (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, content TEXT NOT NULL, source TEXT, category TEXT, status TEXT DEFAULT 'active', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
        ...
        # Create indices
        cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_{self._sanitize_table("baseball_integrations")}_status ON baseball_integrations(status)')
        cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_{self._sanitize_table("baseball_integrations")}_created_at ON baseball_integrations(created_at)')
    ...
    def add_integration(self, title, content, source=None, category=None):
        ...
        cursor = self.conn.cursor()
        cursor.execute("INSERT INTO baseball_integrations (title, content, source, category) VALUES (?, ?, ?, ?)", (title, content, source, category))
        self.conn.commit()
    ...
    def get_integrations(self, status=None, category=None, limit=None):
        ...
        cursor = self.conn.cursor()
        query = 'SELECT * FROM baseball_integrations'
        params = []
    ...
    def get_integration(self, integration_id):
        ...
        cursor = self.conn.cursor()
        cursor.execute('SELECT * FROM baseball_integrations WHERE id = ?', (integration_id,))
    ...
    def update_integration(self, integration_id, **kwargs):
        ...
        if updates:
            query = f"UPDATE baseball_integrations SET {', '.join(updates)} WHERE id = ?"
    ...
    def delete_integration(self, integration_id):
        ...
        cursor = self.conn.cursor()
        cursor.execute('DELETE FROM baseball_integrations WHERE id = ?', (integration_id,))
        self.conn.commit()
...
def main():
    """Main function"""
    agent = BaseballIntegrationAgent()
    try:
        # Example usage
        print(f"Baseball Integration Agent initialized")
        print(f"Database: {agent.db_path}")
        print("Available commands: score, news, stats, schedule, prediction, team, player, history, chart, fan")
    finally:
        agent.close()
