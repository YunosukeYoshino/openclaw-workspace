#!/usr/bin/env python3
"""
Discord Bot Integration for ゲームリプレイ分析エージェント / Game Replay Analysis Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for game-replay-analysis-agent"""
    ...
    def __init__(self, command_prefix: str = "!", db=None):
        ...
        self.db = db
        self.agent_id = "game-replay-analysis-agent"

    async def setup_hook(self):
        """Bot setup"""
        logger.info(f"Setting up {self.agent_id} Discord bot...")
        await self.add_cog(GameReplayAnalysisAgentCommands(self))
...
class GameReplayAnalysisAgentCommands(commands.Cog):
    """Commands for game-replay-analysis-agent"""
...
"""
        help_text = help_text.replace("game-replay-analysis-agent", agent['id'])
        help_text = help_text.replace("ゲームリプレイ分析エージェント", agent['name_ja'])
        help_text = help_text.replace("            - Replay Parsing
            - Key Moments
            - Pattern Recognition
            - Improvement Suggest
            - Pro Comparison
            - Skill Gap Analysis", features_list)
        help_text = help_text.replace("GameReplayAnalysisAgent", snake_to_camel(agent['id']))
