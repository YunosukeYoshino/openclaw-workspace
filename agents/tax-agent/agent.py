...
class TaxAgent:
    """TaxAgent - SQLite ベースの税金記録管理エージェント。自然言語解析によるメッセージ処理をサポートし、日本語と英語に対応しています。"""

    def __init__(self, db_path: str = None):
        """初期化"""
        if db_path is None:
            db_path = Path(__file__).parent / "tax-agent.db"
        self.db = Database(str(db_path))
        self.table_name = "tax_agent"
