...
class KeyManagementAgentDB:
    """キー管理エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/key-management-agent.db"):
...
async def main():
    """動作確認"""
    db = KeyManagementAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for キー管理エージェント"
    )
