...
レポート生成エージェント。レポートの自動生成。

## 概要

このエージェントは レポート生成エージェント。レポートの自動生成。 ためのAIアシスタントです。
...
```python
from db import ReportGeneratorAgentDB
...
### ReportGeneratorAgent.process_task(task)
...
### ReportGeneratorAgentDB.insert_data(data_type, content, metadata)
...
### ReportGeneratorAgentDB.query_data(data_type, limit)
