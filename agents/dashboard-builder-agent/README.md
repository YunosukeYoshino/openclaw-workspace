...
ダッシュボードビルダーエージェント。ダッシュボードの作成・管理。

## 概要

このエージェントは ダッシュボードビルダーエージェント。ダッシュボードの作成・管理。 ためのAIアシスタントです。
...
```python
from agent import DashboardBuilderAgent
...
```python
from db import DashboardBuilderAgentDB
...
### DashboardBuilderAgent.process_task(task)
...
### DashboardBuilderAgentDB.insert_data(data_type, content, metadata)
...
### DashboardBuilderAgentDB.query_data(data_type, limit)
