...
チャート生成エージェント。グラフ・チャートの生成。

## 概要

このエージェントは チャート生成エージェント。グラフ・チャートの生成。 ためのAIアシスタントです。
...
```python
from agent import ChartGeneratorAgent
...
```python
from db import ChartGeneratorAgentDB

db = ChartGeneratorAgentDB()
...
### ChartGeneratorAgent.process_task(task)
...
### ChartGeneratorAgentDB.insert_data(data_type, content, metadata)
...
### ChartGeneratorAgentDB.query_data(data_type, limit)
