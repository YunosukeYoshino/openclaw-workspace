...
このエージェントは えっちソーシャルアナリティクスエージェント。ソーシャル指標の分析。 ためのAIアシスタントです。
...
```python
from agent import EroticSocialAnalyticsAgent
...
```python
from db import EroticSocialAnalyticsAgentDB
...
### EroticSocialAnalyticsAgent.process_task(task)
...
### EroticSocialAnalyticsAgentDB.insert_data(data_type, content, metadata)
...
### EroticSocialAnalyticsAgentDB.query_data(data_type, limit)
