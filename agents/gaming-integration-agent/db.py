#!/usr/bin/env python3
"""
Database module for Gaming Integration Agent
ゲーム統合エージェント データベースモジュール
"""
...
class GamingIntegrationAgentDB:
    """Database handler for Gaming Integration Agent"""
    ...
    def _init_tables(self) -> None:
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS gaming_integrations (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, content TEXT NOT NULL, source TEXT, category TEXT, status TEXT DEFAULT 'active', metadata TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
        ...
        cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_{self._sanitize_table("gaming_integrations")}_category ON gaming_integrations(category)')
