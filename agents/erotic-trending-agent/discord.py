#!/usr/bin/env python3
"""
Discord Bot module for erotic-trending-agent
えっちコンテンツトレンド分析エージェントのDiscord Botモジュール
"""
...
from .agent import EroticTrendingAgentAgent
from .db import get_db


class DiscordBot(commands.Bot):
    """えっちコンテンツトレンド分析エージェント Discord Bot"""

    def __init__(self, agent: EroticTrendingAgentAgent):
...
def create_bot(agent: EroticTrendingAgentAgent, token: str) -> DiscordBot:
    """Botインスタンスを作成"""
    ...
    async def list_entries(ctx, limit: int = 10):
        ...
        embed = discord.Embed(title="えっちコンテンツトレンド分析エージェント", color=discord.Color.blue())
    ...
    async def help_command(ctx):
        """ヘルプ"""
        embed = discord.Embed(
            title="えっちコンテンツトレンド分析エージェント - ヘルプ",
...
async def run_bot(agent: EroticTrendingAgentAgent, token: str):
    """Botを実行"""
...
def run_bot_sync(token: str):
    """Botを同期的に実行"""
    agent = EroticTrendingAgentAgent()
