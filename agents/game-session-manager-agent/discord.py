#!/usr/bin/env python3
"""
game-session-manager-agent - Discord Botモジュール
"""

import discord
from discord.ext import commands
from db import GameSessionManagerAgentDB

class GameSessionManagerAgentDiscordBot(commands.Bot):
    """game-session-manager-agent Discord Bot"""

    def __init__(self, db_path: str = "game-session-manager-agent.db"):
        intents = discord.Intents.default()
        intents.message_content = True
        super().__init__(command_prefix="!", intents=intents)
        self.db = GameSessionManagerAgentDB(db_path)
...
def main():
    """メイン関数"""
    bot = GameSessionManagerAgentDiscordBot()
