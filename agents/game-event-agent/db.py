#!/usr/bin/env python3
"""
ゲームイベントエージェント データベースモジュール
Game Event Agent Database Module
...
class GameEventAgentDB:
    """ゲームイベントエージェント データベースクラス"""
    ...
    def insert_item(self, name: str, data_json: str = "{}") -> int:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                f"""
                INSERT INTO rewards (name, data_json)
                VALUES (?, ?)
                """,
                (name, data_json)
            )
    ...
    def get_items(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"SELECT * FROM rewards ORDER BY created_at DESC LIMIT ?",
                (limit,)
            )
            rows = cursor.fetchall()
            return [dict(row) for row in rows]
