...
# -*- coding: utf-8 -*-
"""
k8s-cluster-agent - Discord Integration
Discord bot integration for k8s-cluster-agent
"""
...
class K8sClusterAgentDiscord:
    """Discord bot integration for k8s-cluster-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("k8s-cluster-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="k8sclusteragent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="k8s-cluster-agent Status",
                description="K8sクラスターエージェント。Kubernetesクラスターの管理。",
        ...
        @self.bot.command(name="k8sclusteragent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="k8s-cluster-agent Help",
                description="K8sクラスターエージェント。Kubernetesクラスターの管理。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!k8sclusteragent_status` - Show agent status\n`!k8sclusteragent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="k8s-cluster-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = K8sClusterAgentDiscord(bot)
