...
K8sクラスターエージェント。Kubernetesクラスターの管理。

## 概要

このエージェントは K8sクラスターエージェント。Kubernetesクラスターの管理。 ためのAIアシスタントです。
...
```python
from agent import K8sClusterAgent
...
```python
from db import K8sClusterAgentDB

db = K8sClusterAgentDB()
...
### K8sClusterAgent.process_task(task)
...
### K8sClusterAgentDB.insert_data(data_type, content, metadata)
...
### K8sClusterAgentDB.query_data(data_type, limit)
