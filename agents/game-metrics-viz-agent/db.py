...
class GameMetricsVizAgentDB:
    """ゲームメトリクス可視化エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/game-metrics-viz-agent.db"):
...
async def main():
    """動作確認"""
    db = GameMetricsVizAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for ゲームメトリクス可視化エージェント"
    )
