...
class GameAchievementSyncAgentDB:
    """ゲーム実績同期エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/game-achievement-sync-agent.db"):
...
async def main():
    ...
    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for ゲーム実績同期エージェント"
    )
