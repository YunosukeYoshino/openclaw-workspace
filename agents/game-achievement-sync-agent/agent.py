...
class GameAchievementSyncAgent:
    """ゲーム実績同期エージェント"""

    def __init__(self, config: Optional[Dict[str, Any]] = None):
        self.config = config or {}
        self.name = "game-achievement-sync-agent"
        self.title = "ゲーム実績同期エージェント"
        self.description = "ゲーム実績の同期・管理"
...
async def main():
    ...
    sample_task = {
        "id": "sample_001",
        "data": {
            "message": "Sample task for ゲーム実績同期エージェント"
        }
    }
