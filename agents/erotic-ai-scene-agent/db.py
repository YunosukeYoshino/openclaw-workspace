...
class EroticAiSceneAgentDB:
    """えっちAIシーンエージェント データベースクラス"""

    def __init__(self, db_path: str = "erotic-ai-scene-agent.db"):
        """初期化
    ...
    def get_scenes(self, limit: int = 100) -> List[Dict]:
        """scenesリスト取得
        ...
        """
        return self.execute_query(
            f"SELECT * FROM scenes ORDER BY created_at DESC LIMIT ?",
            (limit,)
        )

    def add_scenes(self, **kwargs) -> int:
        """scenes追加

        Returns:
            追加したID
        """
        # scenes_テーブルにデータを追加するロジック
        # 各エージェントの要件に合わせて実装
        pass
