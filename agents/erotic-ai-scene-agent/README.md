# えっちAIシーンエージェント / えっちAIシーンエージェント (EN)

AIによるえっちなシーン・情景描写生成エージェント / AIによるえっちなシーン・情景描写生成エージェント

## Overview / 概要

AIを活用したAIによるえっちなシーン・情景描写生成エージェント。
...
```python
from agent import EroticAiSceneAgent

agent = EroticAiSceneAgent()
...
- `!add_ai-scene <title> <content>` - Add entry / エントリー追加
- `!list_ai-scene [limit]` - List entries / エントリー一覧
- `!get_ai-scene <id>` - Get entry / エントリー取得
- `!search_ai-scene <query>` - Search entries / エントリー検索
- `!stats_ai-scene` - Get statistics / 統計情報
