...
class GameReportingAgentDB:
    """ゲームレポート生成エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/game-reporting-agent.db"):
...
async def main():
    """動作確認"""
    db = GameReportingAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for ゲームレポート生成エージェント"
    )
