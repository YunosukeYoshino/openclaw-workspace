...
class BaseballMetricsAgentDB:
    """野球メトリクス管理エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/baseball-metrics-agent.db"):
...
async def main():
    """動作確認"""
    db = BaseballMetricsAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for 野球メトリクス管理エージェント"
    )
