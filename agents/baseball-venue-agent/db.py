...
class BaseballVenueAgentDB:
    """野球会場エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/baseball-venue-agent.db"):
...
async def main():
    """動作確認"""
    db = BaseballVenueAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for 野球会場エージェント"
    )
