#!/usr/bin/env python3
"""
Discord Bot Integration for ゲームeスポーツカレンダーエージェント / Game Esports Calendar Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for game-esports-calendar-agent"""
    ...
    def __init__(self, command_prefix: str = "!", db=None):
        ...
        self.db = db
        self.agent_id = "game-esports-calendar-agent"

    async def setup_hook(self):
        """Bot setup"""
        logger.info(f"Setting up {self.agent_id} Discord bot...")
        await self.add_cog(GameEsportsCalendarAgentCommands(self))
...
class GameEsportsCalendarAgentCommands(commands.Cog):
    """Commands for game-esports-calendar-agent"""
...
"""
        help_text = help_text.replace("game-esports-calendar-agent", agent['id'])
        help_text = help_text.replace("ゲームeスポーツカレンダーエージェント", agent['name_ja'])
        help_text = help_text.replace("            - Tournament Schedule
            - Qualification Info
            - Reminders
            - Stream Links
            - Multi-Game Support
            - Calendar Export", features_list)
        help_text = help_text.replace("GameEsportsCalendarAgent", snake_to_camel(agent['id']))
