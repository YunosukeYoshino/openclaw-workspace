...
class IdentityVerificationAgentDB:
    """ID認証エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/identity-verification-agent.db"):
...
async def main():
    """動作確認"""
    db = IdentityVerificationAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for ID認証エージェント"
    )
