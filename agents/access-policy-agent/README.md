...
アクセスポリシーエージェント。アクセスポリシーの管理・適用。

## 概要

このエージェントは アクセスポリシーエージェント。アクセスポリシーの管理・適用。 ためのAIアシスタントです。
...
```python
from agent import AccessPolicyAgent
...
```python
from db import AccessPolicyAgentDB

db = AccessPolicyAgentDB()
...
### AccessPolicyAgent.process_task(task)
...
### AccessPolicyAgentDB.insert_data(data_type, content, metadata)
...
### AccessPolicyAgentDB.query_data(data_type, limit)
