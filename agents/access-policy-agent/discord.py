...
# -*- coding: utf-8 -*-
"""
access-policy-agent - Discord Integration
Discord bot integration for access-policy-agent
"""
...
class AccessPolicyAgentDiscord:
    """Discord bot integration for access-policy-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("access-policy-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="accesspolicyagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="access-policy-agent Status",
                description="アクセスポリシーエージェント。アクセスポリシーの管理・適用。",
        ...
        @self.bot.command(name="accesspolicyagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="access-policy-agent Help",
                description="アクセスポリシーエージェント。アクセスポリシーの管理・適用。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!accesspolicyagent_status` - Show agent status\n`!accesspolicyagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="access-policy-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = AccessPolicyAgentDiscord(bot)
