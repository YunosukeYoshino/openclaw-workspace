#!/usr/bin/env python3
"""
Database module for セキュリティネットワークセグメンテーションエージェント
"""
...
class Database:
    """Database handler for セキュリティネットワークセグメンテーションエージェント"""

    def __init__(self, db_path: Optional[str] = None):
        if db_path is None:
            db_path = Path(__file__).parent / "security-network-segmentation-agent.db"
