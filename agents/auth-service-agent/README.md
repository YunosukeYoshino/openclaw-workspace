...
認証サービスエージェント。認証サービスの管理。

## 概要

このエージェントは 認証サービスエージェント。認証サービスの管理。 ためのAIアシスタントです。
...
```python
from agent import AuthServiceAgent
...
```python
from db import AuthServiceAgentDB

db = AuthServiceAgentDB()
...
### AuthServiceAgent.process_task(task)
...
### AuthServiceAgentDB.insert_data(data_type, content, metadata)
...
### AuthServiceAgentDB.query_data(data_type, limit)
