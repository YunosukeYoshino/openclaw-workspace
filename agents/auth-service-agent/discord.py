...
# -*- coding: utf-8 -*-
"""
auth-service-agent - Discord Integration
Discord bot integration for auth-service-agent
"""
...
class AuthServiceAgentDiscord:
    """Discord bot integration for auth-service-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("auth-service-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="authserviceagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="auth-service-agent Status",
                description="認証サービスエージェント。認証サービスの管理。",
        ...
        @self.bot.command(name="authserviceagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="auth-service-agent Help",
                description="認証サービスエージェント。認証サービスの管理。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!authserviceagent_status` - Show agent status\n`!authserviceagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="auth-service-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = AuthServiceAgentDiscord(bot)
