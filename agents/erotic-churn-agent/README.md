...
このエージェントは えっちチャーンエージェント。チャーン（解約）分析・防止。 ためのAIアシスタントです。
...
```python
from agent import EroticChurnAgent
...
```python
from db import EroticChurnAgentDB
...
### EroticChurnAgent.process_task(task)
...
### EroticChurnAgentDB.insert_data(data_type, content, metadata)
...
### EroticChurnAgentDB.query_data(data_type, limit)
