...
監査ポリシーエージェント。監査ポリシーの管理・実施。

## 概要

このエージェントは 監査ポリシーエージェント。監査ポリシーの管理・実施。 ためのAIアシスタントです。
...
```python
from agent import AuditPolicyAgent
...
```python
from db import AuditPolicyAgentDB

db = AuditPolicyAgentDB()
...
### AuditPolicyAgent.process_task(task)
...
### AuditPolicyAgentDB.insert_data(data_type, content, metadata)
...
### AuditPolicyAgentDB.query_data(data_type, limit)
