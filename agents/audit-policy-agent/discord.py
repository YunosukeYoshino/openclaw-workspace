...
# -*- coding: utf-8 -*-
"""
audit-policy-agent - Discord Integration
Discord bot integration for audit-policy-agent
"""
...
class AuditPolicyAgentDiscord:
    """Discord bot integration for audit-policy-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("audit-policy-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="auditpolicyagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="audit-policy-agent Status",
                description="監査ポリシーエージェント。監査ポリシーの管理・実施。",
        ...
        @self.bot.command(name="auditpolicyagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="audit-policy-agent Help",
                description="監査ポリシーエージェント。監査ポリシーの管理・実施。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!auditpolicyagent_status` - Show agent status\n`!auditpolicyagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="audit-policy-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = AuditPolicyAgentDiscord(bot)
