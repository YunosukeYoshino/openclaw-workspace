...
class LearningAgent:
    """LearningAgent - learning agentは、learningの管理と追跡を行うAIエージェントです。"""

    def __init__(self, db_path: str = None):
        """初期化"""
        if db_path is None:
            db_path = Path(__file__).parent / "learning-agent.db"
        self.db = Database(str(db_path))
        self.table_name = "learning_agent"
...
if __name__ == "__main__":
    agent = LearningAgent()
    print(f"{LearningAgent} initialized")
