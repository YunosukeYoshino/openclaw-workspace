...
class GameCoachingAiAgentDB:
    """ゲームコーチングAIエージェント データベースクラス"""

    def __init__(self, db_path: str = "game-coaching-ai-agent.db"):
        """初期化
    ...
    def get_coaching_sessions(self, limit: int = 100) -> List[Dict]:
        """coaching_sessionsリスト取得

        Args:
            limit: 取得件数

        Returns:
            coaching_sessionsリスト
        """
        return self.execute_query(
            f"SELECT * FROM coaching_sessions ORDER BY created_at DESC LIMIT ?",
            (limit,)
        )

    def add_coaching_sessions(self, **kwargs) -> int:
        """coaching_sessions追加

        Returns:
            追加したID
        """
        # coaching_sessions_テーブルにデータを追加するロジック
        # 各エージェントの要件に合わせて実装
        pass
