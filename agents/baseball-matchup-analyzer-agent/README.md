...
このエージェントは 野球対戦分析エージェント。対戦相手の分析・戦略策定。 ためのAIアシスタントです。
...
```python
from agent import BaseballMatchupAnalyzerAgent
...
```python
from db import BaseballMatchupAnalyzerAgentDB
...
### BaseballMatchupAnalyzerAgent.process_task(task)
...
### BaseballMatchupAnalyzerAgentDB.insert_data(data_type, content, metadata)
...
### BaseballMatchupAnalyzerAgentDB.query_data(data_type, limit)
