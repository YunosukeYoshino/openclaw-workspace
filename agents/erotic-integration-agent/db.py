#!/usr/bin/env python3
"""
Database module for Erotic Content Integration Agent
えっちコンテンツ統合エージェント データベースモジュール
"""
...
class EroticIntegrationAgentDB:
    """Database handler for Erotic Content Integration Agent"""
    ...
    def _init_tables(self) -> None:
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS erotic_integrations (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, content TEXT NOT NULL, source TEXT, category TEXT, status TEXT DEFAULT 'active', metadata TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
        ...
        cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_{self._sanitize_table("erotic_integrations")}_category ON erotic_integrations(category)')
