#!/usr/bin/env python3
"""
news agentは、newsの管理と追跡を行うAIエージェントです。
"""
...
class NewsAgent:
    """NewsAgent - news agentは、newsの管理と追跡を行うAIエージェントです。"""

    def __init__(self, db_path: str = None):
        """初期化"""
        if db_path is None:
            db_path = Path(__file__).parent / "news-agent.db"
        self.db = Database(str(db_path))
        self.table_name = "news_agent"
