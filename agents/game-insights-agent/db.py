...
class GameInsightsAgentDB:
    """ゲームインサイト分析エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/game-insights-agent.db"):
...
async def main():
    """動作確認"""
    db = GameInsightsAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for ゲームインサイト分析エージェント"
    )
