...
# -*- coding: utf-8 -*-
"""
game-ml-model-agent - Discord Integration
Discord bot integration for game-ml-model-agent
"""
...
class GameMlModelAgentDiscord:
    """Discord bot integration for game-ml-model-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("game-ml-model-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="gamemlmodelagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="game-ml-model-agent Status",
                description="ゲームMLモデルエージェント。機械学習モデルの管理・訓練。",
        ...
        @self.bot.command(name="gamemlmodelagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="game-ml-model-agent Help",
                description="ゲームMLモデルエージェント。機械学習モデルの管理・訓練。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!gamemlmodelagent_status` - Show agent status\n`!gamemlmodelagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="game-ml-model-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = GameMlModelAgentDiscord(bot)
