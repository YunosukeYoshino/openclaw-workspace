...
ゲームMLモデルエージェント。機械学習モデルの管理・訓練。

## 概要

このエージェントは ゲームMLモデルエージェント。機械学習モデルの管理・訓練。 ためのAIアシスタントです。
...
```python
from agent import GameMlModelAgent

agent = GameMlModelAgent()
...
```python
from db import GameMlModelAgentDB

db = GameMlModelAgentDB()
...
### GameMlModelAgent.process_task(task)
...
### GameMlModelAgentDB.insert_data(data_type, content, metadata)
...
### GameMlModelAgentDB.query_data(data_type, limit)
