...
class BaseballFanLoyaltyAgentDB:
    """野球ファンロイヤルティエージェント データベースクラス"""

    def __init__(self, db_path: str = "data/baseball-fan-loyalty-agent.db"):
...
async def main():
    """動作確認"""
    db = BaseballFanLoyaltyAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for 野球ファンロイヤルティエージェント"
    )
