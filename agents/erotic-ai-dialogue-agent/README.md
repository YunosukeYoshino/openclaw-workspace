# えっちAIダイアログエージェント / えっちAIダイアログエージェント (EN)

AIによるえっちな会話・対話生成エージェント / AIによるえっちな会話・対話生成エージェント

## Overview / 概要

AIを活用したAIによるえっちな会話・対話生成エージェント。
...
```python
from agent import EroticAiDialogueAgent

agent = EroticAiDialogueAgent()
...
- `!add_ai-dialogue <title> <content>` - Add entry / エントリー追加
- `!list_ai-dialogue [limit]` - List entries / エントリー一覧
- `!get_ai-dialogue <id>` - Get entry / エントリー取得
- `!search_ai-dialogue <query>` - Search entries / エントリー検索
- `!stats_ai-dialogue` - Get statistics / 統計情報
