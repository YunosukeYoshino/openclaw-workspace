...
class Data_lake_agentDiscordBot(commands.Bot):
    # data-lake-agent Discord ボット
    ...
    async def setup_hook(self):
        # ボット起動時の設定
        await self.add_cog(Data_lake_agentCommands(self))
...
class Data_lake_agentCommands(commands.Cog):
    # data-lake-agent コマンド

    def __init__(self, bot: commands.Bot):
        # 初期化
        self.bot = bot

    @commands.command(name="data_lake_agent")
    async def data_lake_agent(self, ctx: commands.Context, action: str = "list", *, args: str = ""):
        # メインコマンド
        if action == "list":
            ...
            if not entries:
                ...
                return
            embed = discord.Embed(title="Data Lake Agent 一覧", color=discord.Color.blue())
    ...
    @commands.command(name="data_lake_agent_status")
    async def data_lake_agent_status(self, ctx: commands.Context):
        # ステータス確認
        entries = self.bot.db.list_entries(status="active")
        embed = discord.Embed(title="Data Lake Agent ステータス", color=discord.Color.gold())
        embed.add_field(name="アクティブエントリー", value=str(len(entries)))
        await ctx.send(embed=embed)

    @commands.command(name="data_lake_agent_delete")
    async def data_lake_agent_delete(self, ctx: commands.Context, entry_id: int):
        # エントリー削除
