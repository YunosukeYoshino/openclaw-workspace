#!/usr/bin/env python3
"""
Discord Bot Integration for ゲーム理論エージェント / Game Theory Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for game-theory-agent"""
    ...
    def __init__(self, command_prefix: str = "!", db=None):
        ...
        self.db = db
        self.agent_id = "game-theory-agent"

    async def setup_hook(self):
        """Bot setup"""
        logger.info(f"Setting up {self.agent_id} Discord bot...")
        await self.add_cog(GameTheoryAgentCommands(self))
...
class GameTheoryAgentCommands(commands.Cog):
    """Commands for game-theory-agent"""
...
"""
        help_text = help_text.replace("game-theory-agent", agent['id'])
        help_text = help_text.replace("ゲーム理論エージェント", agent['name_ja'])
        help_text = help_text.replace("            - Decision Analysis
            - Nash Equilibrium
            - Game Theory Models
            - Strategy Opt
            - Payoff Matrix
            - Equilibrium Finder", features_list)
        help_text = help_text.replace("GameTheoryAgent", snake_to_camel(agent['id']))
