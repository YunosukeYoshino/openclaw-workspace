#!/usr/bin/env python3
"""
お気に入りのえっちなキャラ管理エージェント - Discord Botモジュール
...
from db import EroticCharacterAgentDB


class EroticCharacterAgentBot(commands.Bot):
    """お気に入りのえっちなキャラ管理エージェント Discord Bot"""
    ...
    def __init__(self, db_path: str = None, command_prefix: str = "!"):
        ...
        self.db = EroticCharacterAgentDB(db_path)
        self.db.initialize()
...
def get_bot(db_path: str = None, command_prefix: str = "!"):
    ...
    global bot
    if bot is None:
        bot = EroticCharacterAgentBot(db_path, command_prefix)
        ...
        async def show_help(ctx: commands.Context):
            ...
            embed = discord.Embed(
                ...
                description="お気に入りのえっちなキャラ管理エージェントの使い方",
                color=0x00aaff
            )
