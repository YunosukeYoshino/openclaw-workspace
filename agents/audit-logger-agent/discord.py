...
# -*- coding: utf-8 -*-
"""
audit-logger-agent - Discord Integration
Discord bot integration for audit-logger-agent
"""
...
class AuditLoggerAgentDiscord:
    """Discord bot integration for audit-logger-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("audit-logger-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="auditloggeragent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="audit-logger-agent Status",
                description="監査ロガーエージェント。監査ログの記録・管理。",
        ...
        @self.bot.command(name="auditloggeragent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="audit-logger-agent Help",
                description="監査ロガーエージェント。監査ログの記録・管理。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!auditloggeragent_status` - Show agent status\n`!auditloggeragent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="audit-logger-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = AuditLoggerAgentDiscord(bot)
