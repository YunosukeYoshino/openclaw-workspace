...
監査ロガーエージェント。監査ログの記録・管理。

## 概要

このエージェントは 監査ロガーエージェント。監査ログの記録・管理。 ためのAIアシスタントです。
...
```python
from agent import AuditLoggerAgent
...
```python
from db import AuditLoggerAgentDB

db = AuditLoggerAgentDB()
...
### AuditLoggerAgent.process_task(task)
...
### AuditLoggerAgentDB.insert_data(data_type, content, metadata)
...
### AuditLoggerAgentDB.query_data(data_type, limit)
