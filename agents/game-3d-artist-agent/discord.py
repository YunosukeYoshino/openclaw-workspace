...
# -*- coding: utf-8 -*-
"""
game-3d-artist-agent - Discord Integration
Discord bot integration for game-3d-artist-agent
"""
...
class Game3dArtistAgentDiscord:
    """Discord bot integration for game-3d-artist-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("game-3d-artist-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="game3dartistagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="game-3d-artist-agent Status",
                description="ゲーム3Dアーティストエージェント。3Dモデルの制作。",
        ...
        @self.bot.command(name="game3dartistagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="game-3d-artist-agent Help",
                description="ゲーム3Dアーティストエージェント。3Dモデルの制作。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!game3dartistagent_status` - Show agent status\n`!game3dartistagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="game-3d-artist-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = Game3dArtistAgentDiscord(bot)
