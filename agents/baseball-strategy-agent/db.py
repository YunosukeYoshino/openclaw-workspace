#!/usr/bin/env python3
"""
野球戦略分析エージェント データベースモジュール
Baseball Strategy Analysis Agent Database Module
...
class BaseballStrategyAgentDB:
    """野球戦略分析エージェント データベースクラス"""
    ...
    def init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            # インデックス作成
            cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_strategies_status ON strategies(status)")
            cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_strategies_created ON strategies(created_at)")
    ...
    def insert_entry(self, title: str, description: str = "", data_json: str = "{}", status: str = "active") -> int:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                f"""
                INSERT INTO strategies (title, description, data_json, status)
                VALUES (?, ?, ?, ?)
                """,
    ...
    def get_entry(self, entry_id: int) -> Optional[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"SELECT * FROM strategies WHERE id = ?",
                (entry_id,)
            )
            row = cursor.fetchone()
    ...
    def delete_entry(self, entry_id: int) -> bool:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"DELETE FROM strategies WHERE id = ?",
                (entry_id,)
            )
            conn.commit()
            return cursor.rowcount > 0
    ...
    def insert_item(self, name: str, team: str = "", data_json: str = "{}") -> int:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                f"""
                INSERT INTO matches (name, team, data_json)
                VALUES (?, ?, ?)
                """,
                (name, team, data_json)
            )
    ...
    def get_items(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"SELECT * FROM matches ORDER BY created_at DESC LIMIT ?",
                (limit,)
            )
            rows = cursor.fetchall()
            return [dict(row) for row in rows]
