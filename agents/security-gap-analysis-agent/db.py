...
class SecurityGapAnalysisAgentDB:
    """セキュリティギャップ分析エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/security-gap-analysis-agent.db"):
...
async def main():
    """動作確認"""
    db = SecurityGapAnalysisAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for セキュリティギャップ分析エージェント"
    )
