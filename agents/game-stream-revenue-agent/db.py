...
class GameStreamRevenueAgentDB:
    """ゲーム配信収益エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/game-stream-revenue-agent.db"):
...
async def main():
    """動作確認"""
    db = GameStreamRevenueAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for ゲーム配信収益エージェント"
    )
