...
class GamePreservationAgentDB:
    """ゲーム保存エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/game-preservation-agent.db"):
...
async def main():
    """動作確認"""
    db = GamePreservationAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for ゲーム保存エージェント"
    )
