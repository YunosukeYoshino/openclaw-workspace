...
class AccessControlAgentDB:
    """アクセスコントロールエージェント データベースクラス"""

    def __init__(self, db_path: str = "data/access-control-agent.db"):
...
async def main():
    """動作確認"""
    db = AccessControlAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for アクセスコントロールエージェント"
    )
