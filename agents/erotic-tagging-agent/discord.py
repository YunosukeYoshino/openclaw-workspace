...
# -*- coding: utf-8 -*-
"""
erotic-tagging-agent - Discord Integration
Discord bot integration for erotic-tagging-agent
"""
...
class EroticTaggingAgentDiscord:
    """Discord bot integration for erotic-tagging-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("erotic-tagging-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="erotictaggingagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="erotic-tagging-agent Status",
                description="えっちタグ付けエージェント。自動タグ付け・分類。",
        ...
        @self.bot.command(name="erotictaggingagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="erotic-tagging-agent Help",
                description="えっちタグ付けエージェント。自動タグ付け・分類。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!erotictaggingagent_status` - Show agent status\n`!erotictaggingagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="erotic-tagging-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = EroticTaggingAgentDiscord(bot)
