...
このエージェントは データガバナンスエージェント。データガバナンスの実施。 ためのAIアシスタントです。
...
```python
from db import DataGovernanceAgentDB
...
### DataGovernanceAgent.process_task(task)
...
### DataGovernanceAgentDB.insert_data(data_type, content, metadata)
...
### DataGovernanceAgentDB.query_data(data_type, limit)
