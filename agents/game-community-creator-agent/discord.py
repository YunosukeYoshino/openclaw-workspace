#!/usr/bin/env python3
"""
Discord Bot Integration for ゲームコミュニティ作成エージェント / Game Community Creator Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for game-community-creator-agent"""
    ...
    async def setup_hook(self):
        ...
        await self.add_cog(GameCommunityCreatorAgentCommands(self))
...
class GameCommunityCreatorAgentCommands(commands.Cog):
    """Commands for game-community-creator-agent"""
...
"""
        help_text = help_text.replace("game-community-creator-agent", agent['id'])
        help_text = help_text.replace("ゲームコミュニティ作成エージェント", agent['name_ja'])
        help_text = help_text.replace("            - Discord Setup
            - Forum Creation
            - Role Management
            - Channel Config
            - Rule Templates", features_list)
        help_text = help_text.replace("GameCommunityCreatorAgent", snake_to_camel(agent['id']))
