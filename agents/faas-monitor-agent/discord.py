...
# -*- coding: utf-8 -*-
"""
faas-monitor-agent - Discord Integration
Discord bot integration for faas-monitor-agent
"""
...
class FaasMonitorAgentDiscord:
    """Discord bot integration for faas-monitor-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("faas-monitor-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="faasmonitoragent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="faas-monitor-agent Status",
                description="FaaSモニターエージェント。FaaSの監視・管理。",
        ...
        @self.bot.command(name="faasmonitoragent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="faas-monitor-agent Help",
                description="FaaSモニターエージェント。FaaSの監視・管理。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!faasmonitoragent_status` - Show agent status\n`!faasmonitoragent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="faas-monitor-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = FaasMonitorAgentDiscord(bot)
