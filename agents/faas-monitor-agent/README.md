...
FaaSモニターエージェント。FaaSの監視・管理。

## 概要

このエージェントは FaaSモニターエージェント。FaaSの監視・管理。 ためのAIアシスタントです。
...
```python
from agent import FaasMonitorAgent

agent = FaasMonitorAgent()
...
```python
from db import FaasMonitorAgentDB

db = FaasMonitorAgentDB()
...
### FaasMonitorAgent.process_task(task)
...
### FaasMonitorAgentDB.insert_data(data_type, content, metadata)
...
### FaasMonitorAgentDB.query_data(data_type, limit)
