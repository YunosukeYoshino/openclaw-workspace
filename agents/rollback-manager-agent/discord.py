#!/usr/bin/env python3
"""
rollback-manager-agent - Discord Botモジュール
"""

import discord
from discord.ext import commands
from db import RollbackManagerAgentDB

class RollbackManagerAgentDiscordBot(commands.Bot):
    """rollback-manager-agent Discord Bot"""

    def __init__(self, db_path: str = "rollback-manager-agent.db"):
        intents = discord.Intents.default()
        intents.message_content = True
        super().__init__(command_prefix="!", intents=intents)
        self.db = RollbackManagerAgentDB(db_path)
...
def main():
    """メイン関数"""
    bot = RollbackManagerAgentDiscordBot()
