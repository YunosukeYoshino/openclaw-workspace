#!/usr/bin/env python3
"""
Discord Bot Integration for 野球歴史的名試合エージェント / Baseball Historical Match Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for baseball-historical-match-agent"""
    ...
    def __init__(self, command_prefix: str = "!", db=None):
        ...
        self.db = db
        self.agent_id = "baseball-historical-match-agent"

    async def setup_hook(self):
        """Bot setup"""
        logger.info(f"Setting up {self.agent_id} Discord bot...")
        await self.add_cog(BaseballHistoricalMatchAgentCommands(self))
...
class BaseballHistoricalMatchAgentCommands(commands.Cog):
    """Commands for baseball-historical-match-agent"""
...
"""
        help_text = help_text.replace("baseball-historical-match-agent", agent['id'])
        help_text = help_text.replace("野球歴史的名試合エージェント", agent['name_ja'])
        help_text = help_text.replace("            - Historical Match Records
            - Key Moment Analysis
            - Media Integration
            - Replay Suggestions
            - Commemorative Events
            - Match Search", features_list)
        help_text = help_text.replace("BaseballHistoricalMatchAgent", snake_to_camel(agent['id']))
