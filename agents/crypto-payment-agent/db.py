...
class CryptoPaymentAgentDB:
    """暗号通貨決済エージェント データベースクラス"""

    def __init__(self, db_path: str = "data/crypto-payment-agent.db"):
...
async def main():
    """動作確認"""
    db = CryptoPaymentAgentDB()

    record_id = db.insert_record(
        record_type="sample",
        title="Sample Record",
        content="This is a sample record for 暗号通貨決済エージェント"
    )
