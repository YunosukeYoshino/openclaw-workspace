...
# -*- coding: utf-8 -*-
"""
game-tuning-agent - Discord Integration
Discord bot integration for game-tuning-agent
"""
...
class GameTuningAgentDiscord:
    """Discord bot integration for game-tuning-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("game-tuning-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="gametuningagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="game-tuning-agent Status",
                description="ゲームチューニングエージェント。ゲームパラメータの調整・最適化。",
        ...
        @self.bot.command(name="gametuningagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="game-tuning-agent Help",
                description="ゲームチューニングエージェント。ゲームパラメータの調整・最適化。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!gametuningagent_status` - Show agent status\n`!gametuningagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="game-tuning-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = GameTuningAgentDiscord(bot)
