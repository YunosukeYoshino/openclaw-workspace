#!/usr/bin/env python3
"""
Discord Bot Integration for 野球イニング戦略エージェント / Baseball Inning Strategy Agent
"""
...
class DiscordBot(commands.Bot):
    """Discord Bot for baseball-inning-strategy-agent"""
    ...
    def __init__(self, command_prefix: str = "!", db=None):
        ...
        self.db = db
        self.agent_id = "baseball-inning-strategy-agent"

    async def setup_hook(self):
        """Bot setup"""
        logger.info(f"Setting up {self.agent_id} Discord bot...")
        await self.add_cog(BaseballInningStrategyAgentCommands(self))
...
class BaseballInningStrategyAgentCommands(commands.Cog):
    """Commands for baseball-inning-strategy-agent"""
...
"""
        help_text = help_text.replace("baseball-inning-strategy-agent", agent['id'])
        help_text = help_text.replace("野球イニング戦略エージェント", agent['name_ja'])
        help_text = help_text.replace("            - Inning Strategy
            - Situation Analysis
            - Score-Based Tactics
            - Runner Situation
            - Defensive Shifts
            - Relief Timing", features_list)
        help_text = help_text.replace("BaseballInningStrategyAgent", snake_to_camel(agent['id']))
