...
# -*- coding: utf-8 -*-
"""
security-soc-agent - Discord Integration
Discord bot integration for security-soc-agent
"""
...
class SecuritySocAgentDiscord:
    """Discord bot integration for security-soc-agent"""

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.logger = logging.getLogger("security-soc-agent.discord")
    ...
    def setup_commands(self):
        @self.bot.command(name="securitysocagent_status")
        async def agent_status(ctx):
            embed = discord.Embed(
                title="security-soc-agent Status",
                description="セキュリティSOCエージェント。SOC (Security Operations Center) 機能。",
        ...
        @self.bot.command(name="securitysocagent_help")
        async def agent_help(ctx):
            embed = discord.Embed(
                title="security-soc-agent Help",
                description="セキュリティSOCエージェント。SOC (Security Operations Center) 機能。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Commands",
                value="`!securitysocagent_status` - Show agent status\n`!securitysocagent_help` - Show this help message",
                inline=False
            )
            await ctx.send(embed=embed)
    ...
    async def send_alert(self, channel_id: int, title: str, description: str, level: str = "info"):
        ...
        embed.set_footer(text="security-soc-agent")
        return await self.send_notification(channel_id, "", embed)

def setup(bot: commands.Bot):
    discord_integration = SecuritySocAgentDiscord(bot)
