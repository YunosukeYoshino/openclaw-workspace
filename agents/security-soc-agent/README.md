...
このエージェントは セキュリティSOCエージェント。SOC (Security Operations Center) 機能。 ためのAIアシスタントです。
...
```python
from agent import SecuritySocAgent
...
```python
from db import SecuritySocAgentDB

db = SecuritySocAgentDB()
...
### SecuritySocAgent.process_task(task)
...
### SecuritySocAgentDB.insert_data(data_type, content, metadata)
...
### SecuritySocAgentDB.query_data(data_type, limit)
