# ゲームスキル分析エージェント / ゲームスキル分析エージェント (EN)

AIによるプレイヤースキル分析エージェント / AIによるプレイヤースキル分析エージェント

## Overview / 概要

AIを活用したAIによるプレイヤースキル分析エージェント。
...
- `!add_skill-analyzer <title> <content>` - Add entry / エントリー追加
- `!list_skill-analyzer [limit]` - List entries / エントリー一覧
- `!get_skill-analyzer <id>` - Get entry / エントリー取得
- `!search_skill-analyzer <query>` - Search entries / エントリー検索
- `!stats_skill-analyzer` - Get statistics / 統計情報
