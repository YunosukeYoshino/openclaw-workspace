#!/usr/bin/env python3
"""Discord Integration for baseball-training-plan-agent"""
...
class BaseballTrainingPlanAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**baseball-training-plan-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for baseball-training-plan-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = BaseballTrainingPlanAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
