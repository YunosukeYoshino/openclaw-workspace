"""
野球投球コーチエージェント - Discord Bot Integration
...
from .db import Baseball_pitching_coach_agentDB
...
class Baseball_pitching_coach_agentBot(commands.Bot):
    """Discord Bot for 野球投球コーチエージェント"""

    def __init__(self, command_prefix: str = "!", db: Optional[Baseball_pitching_coach_agentDB] = None):
        intents = discord.Intents.default()
        intents.message_content = True
        super().__init__(command_prefix=command_prefix, intents=intents)
        self.db = db or Baseball_pitching_coach_agentDB()

    async def setup_hook(self) -> None:
        """Bot起動時のセットアップ"""
        logger.info(f"Setting up {self.__class__.__name__}")
        await self.add_cog(Baseball_pitching_coach_agentCommands(self))
...
class Baseball_pitching_coach_agentCommands(commands.Cog):
    """コマンド定義"""

    def __init__(self, bot: Baseball_pitching_coach_agentBot):
        self.bot = bot
...
async def run_discord_bot(token: str) -> None:
    """Discord Botを実行"""
    bot = Baseball_pitching_coach_agentBot()
    await bot.start(token)

def create_bot(token: str) -> Baseball_pitching_coach_agentBot:
    """Botインスタンスを作成"""
    return Baseball_pitching_coach_agentBot(db=None)
