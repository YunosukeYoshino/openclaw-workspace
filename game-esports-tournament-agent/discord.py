#!/usr/bin/env python3
"""
game-esports-tournament-agent - Discord Botモジュール
"""
...
class GameEsportsTournamentAgentBot(commands.Bot):
    """game-esports-tournament-agent Discord Bot"""
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="game-esports-tournament-agent",
            description="ゲームeスポーツ大会エージェント。大会運営・管理。",
            color=0x00ff00
        )
...
class GameEsportsTournamentAgentCommands(commands.Cog):
    """game-esports-tournament-agent コマンド"""

    def __init__(self, bot: GameEsportsTournamentAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> GameEsportsTournamentAgentBot:
    """Botインスタンス作成"""
    bot = GameEsportsTournamentAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(GameEsportsTournamentAgentCommands(bot))
