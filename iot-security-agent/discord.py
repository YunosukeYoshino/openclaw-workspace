#!/usr/bin/env python3
"""
iot-security-agent - Discord Botモジュール
"""
...
class IotSecurityAgentBot(commands.Bot):
    """iot-security-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="IoTセキュリティエージェント。IoTデバイスのセキュリティ管理。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="iot-security-agent",
            description="IoTセキュリティエージェント。IoTデバイスのセキュリティ管理。",
            color=0x00ff00
        )
...
class IotSecurityAgentCommands(commands.Cog):
    """iot-security-agent コマンド"""

    def __init__(self, bot: IotSecurityAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> IotSecurityAgentBot:
    """Botインスタンス作成"""
    bot = IotSecurityAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(IotSecurityAgentCommands(bot))
