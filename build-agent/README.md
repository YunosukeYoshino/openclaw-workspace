...
ビルドエージェント。ビルドプロセスの管理。

Build agent. Manage build processes.
...
```python
from agent import BuildAgentAgent

agent = BuildAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from build-agent")
```
