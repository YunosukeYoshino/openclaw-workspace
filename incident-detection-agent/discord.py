#!/usr/bin/env python3
"""
incident-detection-agent - Discord Botモジュール
"""
...
class IncidentDetectionAgentBot(commands.Bot):
    """incident-detection-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="インシデント検知エージェント。セキュリティインシデントの検知。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        ...
        embed = discord.Embed(
            ...
            description="インシデント検知エージェント。セキュリティインシデントの検知。",
            color=0x00ff00
        )
...
class IncidentDetectionAgentCommands(commands.Cog):
    """incident-detection-agent コマンド"""

    def __init__(self, bot: IncidentDetectionAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> IncidentDetectionAgentBot:
    """Botインスタンス作成"""
    bot = IncidentDetectionAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(IncidentDetectionAgentCommands(bot))
