...
えっちテイストプロファイルエージェント。趣味プロファイルの作成。

Erotic taste profile agent. Create taste profiles.
...
```python
from agent import EroticTasteProfileAgentAgent

agent = EroticTasteProfileAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from erotic-taste-profile-agent")
```
