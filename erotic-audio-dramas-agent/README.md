...
えっちオーディオドラマエージェント。オーディオドラマの制作。

Erotic audio dramas agent. Create audio dramas.
...
```python
from agent import EroticAudioDramasAgentAgent

agent = EroticAudioDramasAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from erotic-audio-dramas-agent")
```
