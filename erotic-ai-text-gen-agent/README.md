# erotic-ai-text-gen-agent

えっちAIテキスト生成エージェント。AIによる物語・シナリオ生成。

Erotic AI text generation agent. Generate stories and scenarios with AI.
...
```python
from agent import EroticAiTextGenAgentAgent

agent = EroticAiTextGenAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from erotic-ai-text-gen-agent")
```
