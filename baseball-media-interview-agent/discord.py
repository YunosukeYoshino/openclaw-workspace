#!/usr/bin/env python3
"""
baseball-media-interview-agent - Discord Botモジュール
"""
...
class BaseballMediaInterviewAgentBot(commands.Bot):
    """baseball-media-interview-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="野球メディアインタビューエージェント。メディアインタビューの管理・記録。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="baseball-media-interview-agent",
            description="野球メディアインタビューエージェント。メディアインタビューの管理・記録。",
            color=0x00ff00
        )
...
class BaseballMediaInterviewAgentCommands(commands.Cog):
    """baseball-media-interview-agent コマンド"""

    def __init__(self, bot: BaseballMediaInterviewAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> BaseballMediaInterviewAgentBot:
    """Botインスタンス作成"""
    bot = BaseballMediaInterviewAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(BaseballMediaInterviewAgentCommands(bot))
