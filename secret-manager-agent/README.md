...
シークレットマネージャーエージェント。シークレットの管理。

Secret manager agent. Manage secrets.
...
```python
from agent import SecretManagerAgentAgent

agent = SecretManagerAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from secret-manager-agent")
```
