#!/usr/bin/env python3
"""
game-team-manager-agent - Discord Botモジュール
"""
...
class GameTeamManagerAgentBot(commands.Bot):
    """game-team-manager-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="ゲームチームマネージャーエージェント。チーム管理・運営。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="game-team-manager-agent",
            description="ゲームチームマネージャーエージェント。チーム管理・運営。",
            color=0x00ff00
        )
...
class GameTeamManagerAgentCommands(commands.Cog):
    """game-team-manager-agent コマンド"""

    def __init__(self, bot: GameTeamManagerAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> GameTeamManagerAgentBot:
    """Botインスタンス作成"""
    bot = GameTeamManagerAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(GameTeamManagerAgentCommands(bot))
