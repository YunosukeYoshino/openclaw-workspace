#!/usr/bin/env python3
"""
erotic-creator-analytics-agent - Discord Botモジュール
"""
...
class EroticCreatorAnalyticsAgentBot(commands.Bot):
    """erotic-creator-analytics-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="えっちクリエイターアナリティクスエージェント。クリエイターの分析・レポーティング。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="erotic-creator-analytics-agent",
            description="えっちクリエイターアナリティクスエージェント。クリエイターの分析・レポーティング。",
            color=0x00ff00
        )
...
class EroticCreatorAnalyticsAgentCommands(commands.Cog):
    """erotic-creator-analytics-agent コマンド"""

    def __init__(self, bot: EroticCreatorAnalyticsAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> EroticCreatorAnalyticsAgentBot:
    """Botインスタンス作成"""
    bot = EroticCreatorAnalyticsAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(EroticCreatorAnalyticsAgentCommands(bot))
