#!/usr/bin/env python3
"""
erotic-topic-modeling-agent - Discord Botモジュール
"""
...
class EroticTopicModelingAgentBot(commands.Bot):
    """erotic-topic-modeling-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="えっちトピックモデリングエージェント。トピック抽出・分類。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="erotic-topic-modeling-agent",
            description="えっちトピックモデリングエージェント。トピック抽出・分類。",
            color=0x00ff00
        )
...
class EroticTopicModelingAgentCommands(commands.Cog):
    """erotic-topic-modeling-agent コマンド"""

    def __init__(self, bot: EroticTopicModelingAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> EroticTopicModelingAgentBot:
    """Botインスタンス作成"""
    bot = EroticTopicModelingAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(EroticTopicModelingAgentCommands(bot))
