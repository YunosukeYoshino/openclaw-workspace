...
野球スポンサーシップエージェント。スポンサーの管理。

Baseball sponsorship agent. Manage sponsorships.
...
```python
from agent import BaseballSponsorshipAgentAgent

agent = BaseballSponsorshipAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from baseball-sponsorship-agent")
```
