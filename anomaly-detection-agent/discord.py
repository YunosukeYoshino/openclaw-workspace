#!/usr/bin/env python3
"""Discord Integration for anomaly-detection-agent"""
...
class AnomalyDetectionAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**anomaly-detection-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for anomaly-detection-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = AnomalyDetectionAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
