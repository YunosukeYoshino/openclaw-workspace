...
野球ハイライトエージェント。ハイライト映像の作成・管理。

Baseball highlights agent. Create and manage highlight reels.
...
```python
from agent import BaseballHighlightsAgentAgent
...
await bot.connect()
await bot.send_message("Hello from baseball-highlights-agent")
```
