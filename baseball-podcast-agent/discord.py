#!/usr/bin/env python3
"""
baseball-podcast-agent - Discord Botモジュール
"""
...
class BaseballPodcastAgentBot(commands.Bot):
    """baseball-podcast-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="野球ポッドキャストエージェント。ポッドキャストコンテンツの制作・管理。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        ...
        embed = discord.Embed(
            ...
            description="野球ポッドキャストエージェント。ポッドキャストコンテンツの制作・管理。",
            color=0x00ff00
        )
...
class BaseballPodcastAgentCommands(commands.Cog):
    """baseball-podcast-agent コマンド"""

    def __init__(self, bot: BaseballPodcastAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> BaseballPodcastAgentBot:
    """Botインスタンス作成"""
    bot = BaseballPodcastAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(BaseballPodcastAgentCommands(bot))
