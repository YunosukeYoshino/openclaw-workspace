#!/usr/bin/env python3
"""
baseball-video-content-agent - Discord Botモジュール
"""
...
class BaseballVideoContentAgentBot(commands.Bot):
    """baseball-video-content-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="野球ビデオコンテンツエージェント。動画コンテンツの制作・管理。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="baseball-video-content-agent",
            description="野球ビデオコンテンツエージェント。動画コンテンツの制作・管理。",
            color=0x00ff00
        )
...
class BaseballVideoContentAgentCommands(commands.Cog):
    """baseball-video-content-agent コマンド"""

    def __init__(self, bot: BaseballVideoContentAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> BaseballVideoContentAgentBot:
    """Botインスタンス作成"""
    bot = BaseballVideoContentAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(BaseballVideoContentAgentCommands(bot))
