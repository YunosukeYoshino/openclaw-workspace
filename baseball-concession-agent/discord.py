#!/usr/bin/env python3
"""Discord Integration for baseball-concession-agent"""
...
logger = logging.getLogger('baseball-concession-agent')


class BaseballConcessionAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**baseball-concession-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for baseball-concession-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = BaseballConcessionAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
