...
メトリクスコレクターエージェント。メトリクスの収集。

Metrics collector agent. Collect metrics.
...
```python
from agent import MetricsCollectorAgentAgent

agent = MetricsCollectorAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from metrics-collector-agent")
```
