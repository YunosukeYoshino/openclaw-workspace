# game-gameplay-recorder-agent

ゲームゲームプレイレコーダーエージェント。プレイの記録・共有。

Game gameplay recorder agent. Record and share gameplay.
...
```python
from agent import GameGameplayRecorderAgentAgent

agent = GameGameplayRecorderAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from game-gameplay-recorder-agent")
```
