#!/usr/bin/env python3
"""
game-streaming-platform-agent - Discord Botモジュール
"""
...
class GameStreamingPlatformAgentBot(commands.Bot):
    """game-streaming-platform-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="ゲームストリーミングプラットフォームエージェント。ストリーミングプラットフォームの運営。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="game-streaming-platform-agent",
            description="ゲームストリーミングプラットフォームエージェント。ストリーミングプラットフォームの運営。",
            color=0x00ff00
        )
...
class GameStreamingPlatformAgentCommands(commands.Cog):
    """game-streaming-platform-agent コマンド"""

    def __init__(self, bot: GameStreamingPlatformAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> GameStreamingPlatformAgentBot:
    """Botインスタンス作成"""
    bot = GameStreamingPlatformAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(GameStreamingPlatformAgentCommands(bot))
