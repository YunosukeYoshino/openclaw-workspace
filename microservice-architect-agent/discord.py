#!/usr/bin/env python3
"""
microservice-architect-agent - Discord Botモジュール
"""
...
class MicroserviceArchitectAgentBot(commands.Bot):
    """microservice-architect-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="マイクロサービスアーキテクトエージェント。マイクロサービス設計。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="microservice-architect-agent",
            description="マイクロサービスアーキテクトエージェント。マイクロサービス設計。",
            color=0x00ff00
        )
...
class MicroserviceArchitectAgentCommands(commands.Cog):
    """microservice-architect-agent コマンド"""

    def __init__(self, bot: MicroserviceArchitectAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> MicroserviceArchitectAgentBot:
    """Botインスタンス作成"""
    bot = MicroserviceArchitectAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(MicroserviceArchitectAgentCommands(bot))
