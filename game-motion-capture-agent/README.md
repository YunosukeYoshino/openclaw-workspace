...
ゲームモーションキャプチャーエージェント。モーションキャプチャの管理。

Game motion capture agent. Manage motion capture.
...
```python
from agent import GameMotionCaptureAgentAgent

agent = GameMotionCaptureAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from game-motion-capture-agent")
```
