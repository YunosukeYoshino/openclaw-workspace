...
ゲームVR体験エージェント。VR体験の管理。

Game VR experience agent. Manage VR experiences.
...
```python
from agent import GameVrExperienceAgentAgent

agent = GameVrExperienceAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from game-vr-experience-agent")
```
