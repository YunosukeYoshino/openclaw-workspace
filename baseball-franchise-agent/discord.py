...
# -*- coding: utf-8 -*-
"""
Discordボットモジュール - 野球フランチャイズ管理エージェント
"""
...
class DiscordBot(commands.Bot):
    ...
    async def cmd_info(self, ctx: commands.Context):
        embed = discord.Embed(title="野球フランチャイズ管理エージェント", description="野球フランチャイズの情報・歴史を管理するエージェント", color=discord.Color.green())
        embed.add_field(name="カテゴリ", value="野球フランチャイズ", inline=False)
        await ctx.send(embed=embed)
