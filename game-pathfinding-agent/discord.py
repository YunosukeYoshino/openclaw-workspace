#!/usr/bin/env python3
"""Discord Integration for game-pathfinding-agent"""
...
logger = logging.getLogger('game-pathfinding-agent')


class GamePathfindingAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**game-pathfinding-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for game-pathfinding-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = GamePathfindingAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
