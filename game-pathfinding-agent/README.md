...
ゲームパスファインディングエージェント。経路探索。

Game pathfinding agent. Pathfinding.
...
```python
from agent import GamePathfindingAgentAgent

agent = GamePathfindingAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from game-pathfinding-agent")
```
