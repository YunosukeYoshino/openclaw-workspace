...
# -*- coding: utf-8 -*-
"""
Discordボットモジュール - データウェアハウスエージェント
"""
...
class DiscordBot(commands.Bot):
    ...
    async def cmd_info(self, ctx: commands.Context):
        embed = discord.Embed(title="データウェアハウスエージェント", description="データウェアハウスの管理・最適化を行うエージェント", color=discord.Color.green())
        embed.add_field(name="カテゴリ", value="データパイプライン・ETL", inline=False)
        await ctx.send(embed=embed)
