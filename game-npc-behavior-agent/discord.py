#!/usr/bin/env python3
"""Discord Integration for game-npc-behavior-agent"""
...
logger = logging.getLogger('game-npc-behavior-agent')


class GameNpcBehaviorAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**game-npc-behavior-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for game-npc-behavior-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = GameNpcBehaviorAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
