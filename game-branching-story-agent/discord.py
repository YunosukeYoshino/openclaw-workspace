#!/usr/bin/env python3
"""Discord Integration for game-branching-story-agent"""
...
logger = logging.getLogger('game-branching-story-agent')


class GameBranchingStoryAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**game-branching-story-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for game-branching-story-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = GameBranchingStoryAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
