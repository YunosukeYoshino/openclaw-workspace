...
えっちNSFW検知エージェント。NSFWコンテンツの検知。

Erotic NSFW detector agent. Detect NSFW content.
...
```python
from agent import EroticNsfwDetectorAgentAgent

agent = EroticNsfwDetectorAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from erotic-nsfw-detector-agent")
```
