#!/usr/bin/env python3
"""
baseball-media-agent - データベースモジュール
"""
...
class BaseballMediaAgentDatabase:
    """野球メディアエージェント。野球メディアコンテンツの管理。 データベース"""
    ...
    def init_db(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # メインテーブル
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS baseball_media_agent (
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS entry_tags (
                    ...
                    FOREIGN KEY (entry_id) REFERENCES baseball_media_agent(id) ON DELETE CASCADE,
    ...
    def create_entry(self, entry_data: Dict[str, Any]) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO baseball_media_agent (type, title, content, status, priority)
                VALUES (?, ?, ?, ?, ?)
            """, (
    ...
    def get_entry(self, entry_id: int) -> Optional[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM baseball_media_agent WHERE id = ?", (entry_id,))
            row = cursor.fetchone()
            if row:
                entry = dict(row)
    ...
    def list_entries(self, entry_type: Optional[str] = None, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if entry_type:
                cursor.execute("""
                    SELECT * FROM baseball_media_agent
                    WHERE type = ?
    ...
    def delete_entry(self, entry_id: int) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("DELETE FROM baseball_media_agent WHERE id = ?", (entry_id,))
            conn.commit()
            return cursor.rowcount > 0
