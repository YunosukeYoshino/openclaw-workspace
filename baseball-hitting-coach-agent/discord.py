"""
野球打撃コーチエージェント - Discord Bot Integration
打撃フォーム・テクニックのコーチング・分析
"""
...
from .db import Baseball_hitting_coach_agentDB

logger = logging.getLogger('baseball-hitting-coach-agent')

intents = discord.Intents.default()
intents.message_content = True

class Baseball_hitting_coach_agentBot(commands.Bot):
    """Discord Bot for 野球打撃コーチエージェント"""

    def __init__(self, command_prefix: str = "!", db: Optional[Baseball_hitting_coach_agentDB] = None):
        intents = discord.Intents.default()
        intents.message_content = True
        super().__init__(command_prefix=command_prefix, intents=intents)
        self.db = db or Baseball_hitting_coach_agentDB()

    async def setup_hook(self) -> None:
        """Bot起動時のセットアップ"""
        logger.info(f"Setting up {self.__class__.__name__}")
        await self.add_cog(Baseball_hitting_coach_agentCommands(self))
...
class Baseball_hitting_coach_agentCommands(commands.Cog):
    """コマンド定義"""

    def __init__(self, bot: Baseball_hitting_coach_agentBot):
        self.bot = bot
...
async def run_discord_bot(token: str) -> None:
    """Discord Botを実行"""
    bot = Baseball_hitting_coach_agentBot()
    await bot.start(token)

def create_bot(token: str) -> Baseball_hitting_coach_agentBot:
    """Botインスタンスを作成"""
    return Baseball_hitting_coach_agentBot(db=None)
