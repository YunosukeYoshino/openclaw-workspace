#!/usr/bin/env python3
"""
erotic-creator-monetization-agent - Discord Botモジュール
"""
...
class EroticCreatorMonetizationAgentBot(commands.Bot):
    """erotic-creator-monetization-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="えっちクリエイターマネタイゼーションエージェント。収益化支援。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="erotic-creator-monetization-agent",
            description="えっちクリエイターマネタイゼーションエージェント。収益化支援。",
            color=0x00ff00
        )
...
class EroticCreatorMonetizationAgentCommands(commands.Cog):
    """erotic-creator-monetization-agent コマンド"""

    def __init__(self, bot: EroticCreatorMonetizationAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> EroticCreatorMonetizationAgentBot:
    """Botインスタンス作成"""
    bot = EroticCreatorMonetizationAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(EroticCreatorMonetizationAgentCommands(bot))
