# game-tutorial-guide-agent

ゲームチュートリアルガイドエージェント。初心者向けガイドの作成。

Game tutorial guide agent. Create beginner-friendly guides.
...
```python
from agent import GameTutorialGuideAgentAgent

agent = GameTutorialGuideAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from game-tutorial-guide-agent")
```
