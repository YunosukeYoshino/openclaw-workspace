...
ビジネスメトリクスエージェント。KPI等の追跡。

Business metrics agent. Track KPIs and metrics.
...
```python
from agent import BusinessMetricsAgentAgent

agent = BusinessMetricsAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from business-metrics-agent")
```
