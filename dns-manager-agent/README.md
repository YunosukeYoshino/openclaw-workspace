...
DNSマネージャーエージェント。DNSの管理。

DNS manager agent. Manage DNS.
...
```python
from agent import DnsManagerAgentAgent

agent = DnsManagerAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from dns-manager-agent")
```
