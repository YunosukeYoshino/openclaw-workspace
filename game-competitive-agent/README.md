...
ゲームコンペティティブエージェント。競技プレイの管理。

Game competitive agent. Manage competitive play.
...
```python
from agent import GameCompetitiveAgentAgent

agent = GameCompetitiveAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from game-competitive-agent")
```
