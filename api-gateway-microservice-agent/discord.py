...
# -*- coding: utf-8 -*-
"""
Discordボットモジュール - APIゲートウェイマイクロサービスエージェント
"""
...
class DiscordBot(commands.Bot):
    ...
    async def cmd_info(self, ctx: commands.Context):
        embed = discord.Embed(title="APIゲートウェイマイクロサービスエージェント", description="マイクロサービス向けAPIゲートウェイを管理するエージェント", color=discord.Color.green())
        embed.add_field(name="カテゴリ", value="クラウドネイティブ・マイクロサービス", inline=False)
        await ctx.send(embed=embed)
