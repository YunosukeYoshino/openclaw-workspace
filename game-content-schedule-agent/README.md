...
ゲームコンテンツスケジューラーエージェント。配信スケジュールの管理。

Game content scheduler agent. Manage streaming schedules.
...
```python
from agent import GameContentScheduleAgentAgent

agent = GameContentScheduleAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from game-content-schedule-agent")
```
