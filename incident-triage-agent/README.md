...
インシデントトリアージエージェント。インシデントの分類・優先度付け。

Incident triage agent. Classify and prioritize incidents.
...
```python
from agent import IncidentTriageAgentAgent

agent = IncidentTriageAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from incident-triage-agent")
```
