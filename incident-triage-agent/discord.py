#!/usr/bin/env python3
"""
incident-triage-agent - Discord Botモジュール
"""
...
class IncidentTriageAgentBot(commands.Bot):
    """incident-triage-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="インシデントトリアージエージェント。インシデントの分類・優先順位。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        ...
        embed = discord.Embed(
            ...
            description="インシデントトリアージエージェント。インシデントの分類・優先順位。",
            color=0x00ff00
        )
...
class IncidentTriageAgentCommands(commands.Cog):
    """incident-triage-agent コマンド"""

    def __init__(self, bot: IncidentTriageAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> IncidentTriageAgentBot:
    """Botインスタンス作成"""
    bot = IncidentTriageAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(IncidentTriageAgentCommands(bot))
