...
野球マーチャンダイズエージェント。グッズの販売管理。

Baseball merchandise agent. Manage merchandise sales.
...
```python
from agent import BaseballMerchandiseAgentAgent

agent = BaseballMerchandiseAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from baseball-merchandise-agent")
```
