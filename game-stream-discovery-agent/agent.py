...
class GameStreamDiscoveryAgent:
    """game-stream-discovery-agent"""

    def __init__(self, db_path: str = "game-stream-discovery-agent.db"):
...
def main():
    """メイン関数"""
    agent = GameStreamDiscoveryAgent()

    # サンプル実行
    entry_id = agent.add_entry(
        title="サンプル",
        content="ゲームストリームディスカバリーエージェント。ストリームの発見・レコメンド。",
