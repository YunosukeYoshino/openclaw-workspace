#!/usr/bin/env python3
"""
game-stream-discovery-agent - Discord Botモジュール
"""
...
class GameStreamDiscoveryAgentBot(commands.Bot):
    """game-stream-discovery-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="ゲームストリームディスカバリーエージェント。ストリームの発見・レコメンド。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="game-stream-discovery-agent",
            description="ゲームストリームディスカバリーエージェント。ストリームの発見・レコメンド。",
            color=0x00ff00
        )
...
class GameStreamDiscoveryAgentCommands(commands.Cog):
    """game-stream-discovery-agent コマンド"""

    def __init__(self, bot: GameStreamDiscoveryAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> GameStreamDiscoveryAgentBot:
    """Botインスタンス作成"""
    bot = GameStreamDiscoveryAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(GameStreamDiscoveryAgentCommands(bot))
