#!/usr/bin/env python3
"""
incident-communicator-agent - Discord Botモジュール
"""
...
class IncidentCommunicatorAgentBot(commands.Bot):
    """incident-communicator-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="インシデントコミュニケータエージェント。インシデント通信・通知。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="incident-communicator-agent",
            description="インシデントコミュニケータエージェント。インシデント通信・通知。",
            color=0x00ff00
        )
...
class IncidentCommunicatorAgentCommands(commands.Cog):
    """incident-communicator-agent コマンド"""

    def __init__(self, bot: IncidentCommunicatorAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> IncidentCommunicatorAgentBot:
    """Botインスタンス作成"""
    bot = IncidentCommunicatorAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(IncidentCommunicatorAgentCommands(bot))
