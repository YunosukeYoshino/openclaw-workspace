#!/usr/bin/env python3
"""
baseball-documentary-agent - Discord Botモジュール
"""
...
class BaseballDocumentaryAgentBot(commands.Bot):
    """baseball-documentary-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="野球ドキュメンタリーエージェント。ドキュメンタリー制作の管理。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="baseball-documentary-agent",
            description="野球ドキュメンタリーエージェント。ドキュメンタリー制作の管理。",
            color=0x00ff00
        )
...
class BaseballDocumentaryAgentCommands(commands.Cog):
    """baseball-documentary-agent コマンド"""

    def __init__(self, bot: BaseballDocumentaryAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> BaseballDocumentaryAgentBot:
    """Botインスタンス作成"""
    bot = BaseballDocumentaryAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(BaseballDocumentaryAgentCommands(bot))
