...
OAuthプロバイダーエージェント。OAuthの提供。

OAuth provider agent. Provide OAuth.
...
```python
from agent import OauthProviderAgentAgent

agent = OauthProviderAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from oauth-provider-agent")
```
