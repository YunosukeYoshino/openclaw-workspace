#!/usr/bin/env python3
"""
erotic-sentiment-analysis-agent - Discord Botモジュール
"""
...
class EroticSentimentAnalysisAgentBot(commands.Bot):
    """erotic-sentiment-analysis-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="えっちセンチメント分析エージェント。感情分析・評価。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="erotic-sentiment-analysis-agent",
            description="えっちセンチメント分析エージェント。感情分析・評価。",
            color=0x00ff00
        )
...
class EroticSentimentAnalysisAgentCommands(commands.Cog):
    """erotic-sentiment-analysis-agent コマンド"""

    def __init__(self, bot: EroticSentimentAnalysisAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> EroticSentimentAnalysisAgentBot:
    """Botインスタンス作成"""
    bot = EroticSentimentAnalysisAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(EroticSentimentAnalysisAgentCommands(bot))
