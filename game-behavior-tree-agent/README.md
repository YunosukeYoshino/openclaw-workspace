...
ゲームビヘイビアツリーエージェント。行動ツリーの管理。

Game behavior tree agent. Manage behavior trees.
...
```python
from agent import GameBehaviorTreeAgentAgent

agent = GameBehaviorTreeAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from game-behavior-tree-agent")
```
