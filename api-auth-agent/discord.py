#!/usr/bin/env python3
"""Discord Integration for api-auth-agent"""
...
logger = logging.getLogger('api-auth-agent')


class ApiAuthAgentDiscordBot:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for api-auth-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = ApiAuthAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
