...
野球ファシリティエージェント。施設の管理。

Baseball facility agent. Manage facilities.
...
```python
from agent import BaseballFacilityAgentAgent

agent = BaseballFacilityAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from baseball-facility-agent")
```
