...
ネットワークマネージャーエージェント。ネットワークの管理。

Network manager agent. Manage networks.
...
```python
from agent import NetworkManagerAgentAgent

agent = NetworkManagerAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from network-manager-agent")
```
