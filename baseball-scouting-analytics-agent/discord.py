...
# -*- coding: utf-8 -*-
"""
Discordボットモジュール - 野球スカウティング分析エージェント
"""
...
class DiscordBot(commands.Bot):
    ...
    async def cmd_info(self, ctx: commands.Context):
        """エージェント情報を表示"""
        embed = discord.Embed(title="野球スカウティング分析エージェント", description="ドラフト候補選手・移籍対象選手のデータ分析・評価を行うエージェント", color=discord.Color.green())
        embed.add_field(name="カテゴリ", value="野球戦略分析", inline=False)
        await ctx.send(embed=embed)
