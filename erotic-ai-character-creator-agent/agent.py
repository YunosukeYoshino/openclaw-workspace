...
class EroticAiCharacterCreatorAgent(commands.Bot):
    def __init__(self, token):
        intents = discord.Intents.default()
        intents.message_content = True
        super().__init__(command_prefix="!", intents=intents)
        self.token = token
        self.db_path = "erotic-ai-character-creator-agent.db"
        self.init_db()
...
if __name__ == "__main__":
    ...
    if not token:
        ...
        exit(1)
    bot = EroticAiCharacterCreatorAgent(token)
    bot.run_bot()
