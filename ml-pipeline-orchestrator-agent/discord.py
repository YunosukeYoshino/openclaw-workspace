#!/usr/bin/env python3
"""Discord Integration for ml-pipeline-orchestrator-agent"""
...
logger = logging.getLogger('ml-pipeline-orchestrator-agent')


class MlPipelineOrchestratorAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**ml-pipeline-orchestrator-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for ml-pipeline-orchestrator-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = MlPipelineOrchestratorAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
