...
DevOps自動化エージェント。DevOpsプロセスの自動化。

DevOps automation agent. Automate DevOps processes.
...
```python
from agent import DevopsAutomationAgentAgent

agent = DevopsAutomationAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from devops-automation-agent")
```
