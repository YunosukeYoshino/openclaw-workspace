...
野球チケット販売エージェント。チケット販売の管理。

Baseball ticketing agent. Manage ticket sales.
...
```python
from agent import BaseballTicketingAgentAgent

agent = BaseballTicketingAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from baseball-ticketing-agent")
```
