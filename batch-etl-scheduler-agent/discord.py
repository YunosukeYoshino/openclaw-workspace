"""
バッチETLスケジューラーエージェント - Discord Bot Integration
バッチETLジョブのスケジューリング・管理
"""
...
from .db import Batch_etl_scheduler_agentDB

logger = logging.getLogger('batch-etl-scheduler-agent')

intents = discord.Intents.default()
intents.message_content = True

class Batch_etl_scheduler_agentBot(commands.Bot):
    """Discord Bot for バッチETLスケジューラーエージェント"""

    def __init__(self, command_prefix: str = "!", db: Optional[Batch_etl_scheduler_agentDB] = None):
        intents = discord.Intents.default()
        intents.message_content = True
        super().__init__(command_prefix=command_prefix, intents=intents)
        self.db = db or Batch_etl_scheduler_agentDB()

    async def setup_hook(self) -> None:
        """Bot起動時のセットアップ"""
        logger.info(f"Setting up {self.__class__.__name__}")
        await self.add_cog(Batch_etl_scheduler_agentCommands(self))
...
class Batch_etl_scheduler_agentCommands(commands.Cog):
    """コマンド定義"""

    def __init__(self, bot: Batch_etl_scheduler_agentBot):
        self.bot = bot
    ...
    async def status(self, ctx: commands.Context) -> None:
        ...
        embed = discord.Embed(
            title="📊 バッチETLスケジューラーエージェント Status",
...
async def run_discord_bot(token: str) -> None:
    """Discord Botを実行"""
    bot = Batch_etl_scheduler_agentBot()
    await bot.start(token)

def create_bot(token: str) -> Batch_etl_scheduler_agentBot:
    """Botインスタンスを作成"""
    return Batch_etl_scheduler_agentBot(db=None)
