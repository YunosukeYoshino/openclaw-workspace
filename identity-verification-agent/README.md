...
アイデンティティ検証エージェント。身元確認。

Identity verification agent. Verify identity.
...
```python
from agent import IdentityVerificationAgentAgent

agent = IdentityVerificationAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from identity-verification-agent")
```
