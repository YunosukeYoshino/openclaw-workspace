#!/usr/bin/env python3
"""
game-bracket-agent - Discord Botモジュール
"""
...
class GameBracketAgentBot(commands.Bot):
    """game-bracket-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="ゲームブラケットエージェント。トーナメントブラケットの管理。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="game-bracket-agent",
            description="ゲームブラケットエージェント。トーナメントブラケットの管理。",
            color=0x00ff00
        )
...
class GameBracketAgentCommands(commands.Cog):
    """game-bracket-agent コマンド"""

    def __init__(self, bot: GameBracketAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> GameBracketAgentBot:
    """Botインスタンス作成"""
    bot = GameBracketAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(GameBracketAgentCommands(bot))
