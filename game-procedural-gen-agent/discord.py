#!/usr/bin/env python3
"""Discord Integration for game-procedural-gen-agent"""
...
logger = logging.getLogger('game-procedural-gen-agent')


class GameProceduralGenAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**game-procedural-gen-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for game-procedural-gen-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = GameProceduralGenAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
