#!/usr/bin/env python3
"""
erotic-creator-community-agent - Discord Botモジュール
"""
...
class EroticCreatorCommunityAgentBot(commands.Bot):
    """erotic-creator-community-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="えっちクリエイターコミュニティエージェント。クリエイターコミュニティの管理。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="erotic-creator-community-agent",
            description="えっちクリエイターコミュニティエージェント。クリエイターコミュニティの管理。",
            color=0x00ff00
        )
...
class EroticCreatorCommunityAgentCommands(commands.Cog):
    """erotic-creator-community-agent コマンド"""

    def __init__(self, bot: EroticCreatorCommunityAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> EroticCreatorCommunityAgentBot:
    """Botインスタンス作成"""
    bot = EroticCreatorCommunityAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(EroticCreatorCommunityAgentCommands(bot))
