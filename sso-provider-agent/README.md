...
SSOプロバイダーエージェント。シングルサインオンの提供。

SSO provider agent. Provide single sign-on.
...
```python
from agent import SsoProviderAgentAgent

agent = SsoProviderAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from sso-provider-agent")
```
