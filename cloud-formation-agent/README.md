# cloud-formation-agent

CloudFormationエージェント。インフラストラクチャのコード化。

CloudFormation agent. Infrastructure as code.
...
```python
from agent import CloudFormationAgentAgent

agent = CloudFormationAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from cloud-formation-agent")
```
