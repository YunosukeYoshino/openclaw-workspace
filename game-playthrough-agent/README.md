# game-playthrough-agent

ゲームプレイスルーエージェント。完全プレイの記録・管理。

Game playthrough agent. Record and manage complete playthroughs.
...
```python
from agent import GamePlaythroughAgentAgent

agent = GamePlaythroughAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from game-playthrough-agent")
```
