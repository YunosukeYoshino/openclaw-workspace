...
えっちコンテンツ分類エージェント。コンテンツの分類。

Erotic content classifier agent. Classify content.
...
```python
from agent import EroticContentClassifierAgentAgent

agent = EroticContentClassifierAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from erotic-content-classifier-agent")
```
