...
# -*- coding: utf-8 -*-
"""
Discordボットモジュール - えっちクリエイターフィードバックエージェント
"""
...
class DiscordBot(commands.Bot):
    ...
    async def cmd_info(self, ctx: commands.Context):
        embed = discord.Embed(title="えっちクリエイターフィードバックエージェント", description="えっちクリエイターへのフィードバック収集・分析を行うエージェント", color=discord.Color.green())
        embed.add_field(name="カテゴリ", value="えっちコンテンツクリエイターサポート", inline=False)
        await ctx.send(embed=embed)
