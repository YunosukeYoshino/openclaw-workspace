...
データ可視化エージェント。データの可視化。

Data visualization agent. Visualize data.
...
```python
from agent import DataVisualizationAgentAgent

agent = DataVisualizationAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from data-visualization-agent")
```
