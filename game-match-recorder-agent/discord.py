#!/usr/bin/env python3
"""
game-match-recorder-agent - Discord Botモジュール
"""
...
class GameMatchRecorderAgentBot(commands.Bot):
    """game-match-recorder-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="ゲームマッチレコーダーエージェント。試合記録の管理。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="game-match-recorder-agent",
            description="ゲームマッチレコーダーエージェント。試合記録の管理。",
            color=0x00ff00
        )
...
class GameMatchRecorderAgentCommands(commands.Cog):
    """game-match-recorder-agent コマンド"""

    def __init__(self, bot: GameMatchRecorderAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> GameMatchRecorderAgentBot:
    """Botインスタンス作成"""
    bot = GameMatchRecorderAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(GameMatchRecorderAgentCommands(bot))
