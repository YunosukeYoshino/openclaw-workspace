#!/usr/bin/env python3
"""Discord Integration for game-world-lore-agent"""
...
logger = logging.getLogger('game-world-lore-agent')


class GameWorldLoreAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**game-world-lore-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for game-world-lore-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = GameWorldLoreAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
