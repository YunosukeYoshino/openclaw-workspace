#!/usr/bin/env python3
"""
consent-manager-agent - Discord Botモジュール
"""
...
class ConsentManagerAgentBot(commands.Bot):
    """consent-manager-agent Discord Bot"""
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="consent-manager-agent",
            description="同意管理エージェント。ユーザー同意の管理・記録。",
            color=0x00ff00
        )
...
class ConsentManagerAgentCommands(commands.Cog):
    """consent-manager-agent コマンド"""

    def __init__(self, bot: ConsentManagerAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> ConsentManagerAgentBot:
    """Botインスタンス作成"""
    bot = ConsentManagerAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(ConsentManagerAgentCommands(bot))
