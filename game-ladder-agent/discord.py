#!/usr/bin/env python3
"""
game-ladder-agent - Discord Botモジュール
"""
...
class GameLadderAgentBot(commands.Bot):
    """game-ladder-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="ゲームラダーエージェント。ランキング・ラダーの管理。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="game-ladder-agent",
            description="ゲームラダーエージェント。ランキング・ラダーの管理。",
            color=0x00ff00
        )
...
class GameLadderAgentCommands(commands.Cog):
    """game-ladder-agent コマンド"""

    def __init__(self, bot: GameLadderAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> GameLadderAgentBot:
    """Botインスタンス作成"""
    bot = GameLadderAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(GameLadderAgentCommands(bot))
