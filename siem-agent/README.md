...
SIEMエージェント。セキュリティ情報イベント管理。

SIEM agent. Security Information and Event Management.
...
```python
from agent import SiemAgentAgent

agent = SiemAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from siem-agent")
```
