#!/usr/bin/env python3
"""Discord Integration for stream-donation-agent"""
...
class StreamDonationAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**stream-donation-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for stream-donation-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = StreamDonationAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
