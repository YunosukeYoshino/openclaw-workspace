...
ゲームAR体験エージェント。AR体験の管理。

Game AR experience agent. Manage AR experiences.
...
```python
from agent import GameArExperienceAgentAgent

agent = GameArExperienceAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from game-ar-experience-agent")
```
