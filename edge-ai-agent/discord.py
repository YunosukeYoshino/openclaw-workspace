#!/usr/bin/env python3
"""
edge-ai-agent - Discord Botモジュール
"""
...
class EdgeAiAgentBot(commands.Bot):
    """edge-ai-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="エッジAIエージェント。エッジデバイスでのAI推論管理。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        ...
        embed = discord.Embed(
            title="edge-ai-agent",
            description="エッジAIエージェント。エッジデバイスでのAI推論管理。",
            color=0x00ff00
        )
...
class EdgeAiAgentCommands(commands.Cog):
    """edge-ai-agent コマンド"""

    def __init__(self, bot: EdgeAiAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> EdgeAiAgentBot:
    """Botインスタンス作成"""
    bot = EdgeAiAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(EdgeAiAgentCommands(bot))
