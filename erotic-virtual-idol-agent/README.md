...
えっちバーチャルアイドルエージェント。バーチャルアイドルの管理。

Erotic virtual idol agent. Manage virtual idols.
...
```python
from agent import EroticVirtualIdolAgentAgent

agent = EroticVirtualIdolAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from erotic-virtual-idol-agent")
```
