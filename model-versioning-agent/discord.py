#!/usr/bin/env python3
"""Discord Integration for model-versioning-agent"""
...
logger = logging.getLogger('model-versioning-agent')


class ModelVersioningAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**model-versioning-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for model-versioning-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = ModelVersioningAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
