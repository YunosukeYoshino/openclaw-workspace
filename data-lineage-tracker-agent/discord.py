"""
データリネージトラッカーエージェント - Discord Bot Integration
データリネージ（系譜）の追跡・可視化
"""
...
from .db import Data_lineage_tracker_agentDB
...
class Data_lineage_tracker_agentBot(commands.Bot):
    """Discord Bot for データリネージトラッカーエージェント"""

    def __init__(self, command_prefix: str = "!", db: Optional[Data_lineage_tracker_agentDB] = None):
        intents = discord.Intents.default()
        intents.message_content = True
        super().__init__(command_prefix=command_prefix, intents=intents)
        self.db = db or Data_lineage_tracker_agentDB()

    async def setup_hook(self) -> None:
        """Bot起動時のセットアップ"""
        logger.info(f"Setting up {self.__class__.__name__}")
        await self.add_cog(Data_lineage_tracker_agentCommands(self))
...
class Data_lineage_tracker_agentCommands(commands.Cog):
    """コマンド定義"""

    def __init__(self, bot: Data_lineage_tracker_agentBot):
        self.bot = bot
    ...
    async def status(self, ctx: commands.Context) -> None:
        ...
        embed = discord.Embed(
            title="📊 データリネージトラッカーエージェント Status",
...
async def run_discord_bot(token: str) -> None:
    """Discord Botを実行"""
    bot = Data_lineage_tracker_agentBot()
    await bot.start(token)

def create_bot(token: str) -> Data_lineage_tracker_agentBot:
    """Botインスタンスを作成"""
    return Data_lineage_tracker_agentBot(db=None)
