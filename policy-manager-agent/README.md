...
ポリシーマネージャーエージェント。ポリシーの管理。

Policy manager agent. Manage policies.
...
```python
from agent import PolicyManagerAgentAgent

agent = PolicyManagerAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from policy-manager-agent")
```
