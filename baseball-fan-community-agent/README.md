...
野球ファンコミュニティエージェント。ファンコミュニティの構築。

Baseball fan community agent. Build fan communities.
...
```python
from agent import BaseballFanCommunityAgentAgent

agent = BaseballFanCommunityAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from baseball-fan-community-agent")
```
