...
ゲームパーティマネージャーエージェント。パーティの管理。

Game party manager agent. Manage game parties.
...
```python
from agent import GamePartyManagerAgentAgent

agent = GamePartyManagerAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from game-party-manager-agent")
```
