#!/usr/bin/env python3
"""Discord Integration for report-agent"""
...
class ReportAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**report-agent Commands:**

`{self.prefix}add <type> <content>` - Add a report
`{self.prefix}get <id>` - Get a report by ID
`{self.prefix}list [type]` - List reports
`{self.prefix}update <id> <status>` - Update report status
`{self.prefix}delete <id>` - Delete a report
`{self.prefix}stats` - Show statistics

**Entry types:** daily, weekly, monthly, summary, analysis
...
"""
        ...
        async def add_command(ctx, entry_type: str, *, content: str):
            try:
                ...
                if entry_id:
                    await ctx.send(f"Added {entry_type} report (ID: {entry_id})")
                else:
                    await ctx.send("Failed to add report")
        ...
        async def get_command(ctx, entry_id: int):
            try:
                ...
                if entry:
                    embed = discord.Embed(title=f"Report #{entry_id}", description=entry.get('content', ''), color=discord.Color.green())
                    ...
                    await ctx.send(f"Report {entry_id} not found")
            except Exception as err:
            ...
            try:
                ...
                if entries:
                    ...
                    for entry in entries[:10]:
                        msg += f"#{entry['id']} {entry.get('type', 'report')} - {entry.get('content', '')[:50]}...\n"
                    if len(entries) > 10:
        ...
        async def update_command(ctx, entry_id: int, new_status: str):
            try:
                ...
                if result:
                    await ctx.send(f"Updated report {entry_id} to {new_status}")
                else:
                    await ctx.send("Failed to update report")
            except Exception as err:
        ...
        async def delete_command(ctx, entry_id: int):
            try:
                ...
                if result:
                    await ctx.send(f"Deleted report {entry_id}")
                else:
...
async def main():
    ...
    bot = ReportAgentDiscordBot(token=token, prefix=prefix)
    await bot.start()
