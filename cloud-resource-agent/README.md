...
クラウドリソースエージェント。クラウドリソースの管理。

Cloud resource agent. Manage cloud resources.
...
```python
from agent import CloudResourceAgentAgent

agent = CloudResourceAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from cloud-resource-agent")
```
