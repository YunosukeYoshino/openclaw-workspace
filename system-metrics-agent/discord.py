#!/usr/bin/env python3
"""Discord Integration for system-metrics-agent"""
...
class SystemMetricsAgentDiscordBot:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for system-metrics-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = SystemMetricsAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
