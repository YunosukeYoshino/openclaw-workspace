#!/usr/bin/env python3
"""
baseball-exchange-program-agent - Discord Botモジュール
"""
...
class BaseballExchangeProgramAgentBot(commands.Bot):
    """baseball-exchange-program-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="野球交換プログラムエージェント。選手交換プログラムの管理。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="baseball-exchange-program-agent",
            description="野球交換プログラムエージェント。選手交換プログラムの管理。",
            color=0x00ff00
        )
...
class BaseballExchangeProgramAgentCommands(commands.Cog):
    """baseball-exchange-program-agent コマンド"""

    def __init__(self, bot: BaseballExchangeProgramAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> BaseballExchangeProgramAgentBot:
    """Botインスタンス作成"""
    bot = BaseballExchangeProgramAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(BaseballExchangeProgramAgentCommands(bot))
