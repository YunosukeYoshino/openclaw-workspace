...
暗号化マネージャーエージェント。暗号化の管理。

Encryption manager agent. Manage encryption.
...
```python
from agent import EncryptionManagerAgentAgent

agent = EncryptionManagerAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from encryption-manager-agent")
```
