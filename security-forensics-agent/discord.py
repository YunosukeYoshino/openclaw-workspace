...
# -*- coding: utf-8 -*-
"""
Discordボットモジュール - セキュリティフォレンジックエージェント
"""
...
class DiscordBot(commands.Bot):
    ...
    async def cmd_info(self, ctx: commands.Context):
        embed = discord.Embed(title="セキュリティフォレンジックエージェント", description="セキュリティインシデントのフォレンジック調査を行うエージェント", color=discord.Color.green())
        embed.add_field(name="カテゴリ", value="セキュリティフォレンジック", inline=False)
        await ctx.send(embed=embed)
