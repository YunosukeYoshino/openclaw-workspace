...
えっち自動タグエージェント。自動タグ付け。

Erotic auto tag agent. Automatic content tagging.
...
```python
from agent import EroticAutoTagAgentAgent

agent = EroticAutoTagAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from erotic-auto-tag-agent")
```
