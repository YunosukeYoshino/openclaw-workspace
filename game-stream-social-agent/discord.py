#!/usr/bin/env python3
"""
game-stream-social-agent - Discord Botモジュール
"""
...
class GameStreamSocialAgentBot(commands.Bot):
    """game-stream-social-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="ゲームストリームソーシャルエージェント。ストリームのソーシャル機能管理。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="game-stream-social-agent",
            description="ゲームストリームソーシャルエージェント。ストリームのソーシャル機能管理。",
            color=0x00ff00
        )
...
class GameStreamSocialAgentCommands(commands.Cog):
    """game-stream-social-agent コマンド"""

    def __init__(self, bot: GameStreamSocialAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> GameStreamSocialAgentBot:
    """Botインスタンス作成"""
    bot = GameStreamSocialAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(GameStreamSocialAgentCommands(bot))
