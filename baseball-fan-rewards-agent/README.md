...
野球ファンリワードエージェント。ファン報酬プログラム。

Baseball fan rewards agent. Fan rewards program.
...
```python
from agent import BaseballFanRewardsAgentAgent

agent = BaseballFanRewardsAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from baseball-fan-rewards-agent")
```
