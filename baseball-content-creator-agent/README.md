...
野球コンテンツクリエイターエージェント。オリジナルコンテンツの制作。

Baseball content creator agent. Create original content.
...
```python
from agent import BaseballContentCreatorAgentAgent

agent = BaseballContentCreatorAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from baseball-content-creator-agent")
```
