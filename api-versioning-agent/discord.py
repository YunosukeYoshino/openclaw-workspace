#!/usr/bin/env python3
"""Discord Integration for api-versioning-agent"""
...
logger = logging.getLogger('api-versioning-agent')


class ApiVersioningAgentDiscordBot:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for api-versioning-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = ApiVersioningAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
