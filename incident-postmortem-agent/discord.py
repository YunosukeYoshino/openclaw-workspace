#!/usr/bin/env python3
"""
incident-postmortem-agent - Discord Botモジュール
"""
...
class IncidentPostmortemAgentBot(commands.Bot):
    """incident-postmortem-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="インシデントポストモーテムエージェント。インシデント事後分析。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        ...
        embed = discord.Embed(
            ...
            description="インシデントポストモーテムエージェント。インシデント事後分析。",
            color=0x00ff00
        )
...
class IncidentPostmortemAgentCommands(commands.Cog):
    """incident-postmortem-agent コマンド"""

    def __init__(self, bot: IncidentPostmortemAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> IncidentPostmortemAgentBot:
    """Botインスタンス作成"""
    bot = IncidentPostmortemAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(IncidentPostmortemAgentCommands(bot))
