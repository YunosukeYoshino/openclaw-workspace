...
野球コメンタリーエージェント。実況・解説の管理。

Baseball commentator agent. Manage commentary and analysis.
...
```python
from agent import BaseballCommentatorAgentAgent

agent = BaseballCommentatorAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from baseball-commentator-agent")
```
