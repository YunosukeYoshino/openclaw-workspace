#!/usr/bin/env python3
"""
game-stream-analytics-agent - Discord Botモジュール
"""
...
class GameStreamAnalyticsAgentBot(commands.Bot):
    """game-stream-analytics-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="ゲームストリームアナリティクスエージェント。ストリーム分析・統計。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="game-stream-analytics-agent",
            description="ゲームストリームアナリティクスエージェント。ストリーム分析・統計。",
            color=0x00ff00
        )
...
class GameStreamAnalyticsAgentCommands(commands.Cog):
    """game-stream-analytics-agent コマンド"""

    def __init__(self, bot: GameStreamAnalyticsAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> GameStreamAnalyticsAgentBot:
    """Botインスタンス作成"""
    bot = GameStreamAnalyticsAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(GameStreamAnalyticsAgentCommands(bot))
