# erotic-prompt-engineer-agent

えっちプロンプトエンジニアエージェント。AIプロンプトの最適化。

Erotic prompt engineer agent. Optimize AI prompts.
...
```python
from agent import EroticPromptEngineerAgentAgent

agent = EroticPromptEngineerAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from erotic-prompt-engineer-agent")
```
