#!/usr/bin/env python3
"""Discord Integration for game-ai-dialogue-agent"""
...
logger = logging.getLogger('game-ai-dialogue-agent')


class GameAiDialogueAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**game-ai-dialogue-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for game-ai-dialogue-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = GameAiDialogueAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
