#!/usr/bin/env python3
"""Discord Integration for ml-experiment-tracker-agent"""
...
logger = logging.getLogger('ml-experiment-tracker-agent')


class MlExperimentTrackerAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**ml-experiment-tracker-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for ml-experiment-tracker-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = MlExperimentTrackerAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
