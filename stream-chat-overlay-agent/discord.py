#!/usr/bin/env python3
"""Discord Integration for stream-chat-overlay-agent"""
...
logger = logging.getLogger('stream-chat-overlay-agent')


class StreamChatOverlayAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**stream-chat-overlay-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for stream-chat-overlay-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = StreamChatOverlayAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
