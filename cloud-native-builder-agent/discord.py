#!/usr/bin/env python3
"""
cloud-native-builder-agent - Discord Botモジュール
"""
...
class CloudNativeBuilderAgentBot(commands.Bot):
    """cloud-native-builder-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="クラウドネイティブビルダーエージェント。クラウドネイティブアプリのビルド。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="cloud-native-builder-agent",
            description="クラウドネイティブビルダーエージェント。クラウドネイティブアプリのビルド。",
            color=0x00ff00
        )
...
class CloudNativeBuilderAgentCommands(commands.Cog):
    """cloud-native-builder-agent コマンド"""

    def __init__(self, bot: CloudNativeBuilderAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> CloudNativeBuilderAgentBot:
    """Botインスタンス作成"""
    bot = CloudNativeBuilderAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(CloudNativeBuilderAgentCommands(bot))
