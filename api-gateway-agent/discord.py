#!/usr/bin/env python3
"""Discord Integration for api-gateway-agent"""
...
class ApiGatewayAgentDiscordBot:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for api-gateway-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = ApiGatewayAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
