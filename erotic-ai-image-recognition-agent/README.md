...
えっちAI画像認識エージェント。AIによる画像認識。

Erotic AI image recognition agent. Recognize images with AI.
...
```python
from agent import EroticAiImageRecognitionAgentAgent

agent = EroticAiImageRecognitionAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from erotic-ai-image-recognition-agent")
```
