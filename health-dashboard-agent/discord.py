#!/usr/bin/env python3
"""Discord Integration for health-dashboard-agent"""
...
logger = logging.getLogger('health-dashboard-agent')


class HealthDashboardAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**health-dashboard-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for health-dashboard-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = HealthDashboardAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
