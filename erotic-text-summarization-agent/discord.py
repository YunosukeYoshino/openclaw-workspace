#!/usr/bin/env python3
"""
erotic-text-summarization-agent - Discord Botモジュール
"""
...
class EroticTextSummarizationAgentBot(commands.Bot):
    """erotic-text-summarization-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="えっちテキスト要約エージェント。テキストの要約・生成。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="erotic-text-summarization-agent",
            description="えっちテキスト要約エージェント。テキストの要約・生成。",
            color=0x00ff00
        )
...
class EroticTextSummarizationAgentCommands(commands.Cog):
    """erotic-text-summarization-agent コマンド"""

    def __init__(self, bot: EroticTextSummarizationAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> EroticTextSummarizationAgentBot:
    """Botインスタンス作成"""
    bot = EroticTextSummarizationAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(EroticTextSummarizationAgentCommands(bot))
