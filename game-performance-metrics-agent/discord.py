...
# -*- coding: utf-8 -*-
"""
Discordボットモジュール - ゲームパフォーマンスメトリクスエージェント
"""
...
class DiscordBot(commands.Bot):
    ...
    async def cmd_info(self, ctx: commands.Context):
        embed = discord.Embed(title="ゲームパフォーマンスメトリクスエージェント", description="ゲームパフォーマンス（FPS、ラグなど）を監視するエージェント", color=discord.Color.green())
        embed.add_field(name="カテゴリ", value="ゲーム分析・メトリクス", inline=False)
        await ctx.send(embed=embed)
