#!/usr/bin/env python3
"""
data-rights-agent - Discord Botモジュール
"""
...
class DataRightsAgentBot(commands.Bot):
    """data-rights-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="データ権利エージェント。データ権利要求の対応・管理。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="data-rights-agent",
            description="データ権利エージェント。データ権利要求の対応・管理。",
            color=0x00ff00
        )
...
class DataRightsAgentCommands(commands.Cog):
    """data-rights-agent コマンド"""

    def __init__(self, bot: DataRightsAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> DataRightsAgentBot:
    """Botインスタンス作成"""
    bot = DataRightsAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(DataRightsAgentCommands(bot))
