#!/usr/bin/env python3
"""Discord Integration for baseball-historical-match-agent"""
...
class BaseballHistoricalMatchAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**baseball-historical-match-agent Commands:**

`{self.prefix}add <title> <content>` - Add a historical match
`{self.prefix}get <id>` - Get a match by ID
`{self.prefix}list [type]` - List matches
`{self.prefix}search <query>` - Search matches
`{self.prefix}stats` - Show statistics
`{self.prefix}hof` - List legendary matches

**Match types:** world_series, all_star, playoff, regular_season
**Importance levels:** legendary, historic, memorable
"""
            embed = discord.Embed(title="Bot Commands", description=help_text, color=discord.Color.blue())
            await ctx.send(embed=embed)

        @self.bot.command(name="add")
        async def add_command(ctx, title: str, *, content: str):
            try:
                from db import DatabaseManager
                db = DatabaseManager(self.db_path)
                await db.connect()
                entry_id = await db.add_entry("match", title, content)
                await db.close()

                if entry_id:
                    await ctx.send(f"Added match (ID: {entry_id}): {title}")
                else:
                    await ctx.send("Failed to add match")
        ...
        async def get_command(ctx, entry_id: int):
            try:
                ...
                if entry:
                    embed = discord.Embed(title=f"Match #{entry_id}", description=entry.get('content', ''), color=discord.Color.green())
                    ...
                    await ctx.send(f"Match {entry_id} not found")
            except Exception as err:
                logger.error(f"Get command error: {err}")
                await ctx.send(f"Error: {err}")

        @self.bot.command(name="list")
        async def list_command(ctx, match_type: str = None, limit: int = 20):
            try:
                from db import DatabaseManager
                db = DatabaseManager(self.db_path)
                await db.connect()
                entries = await db.list_entries(match_type, limit=limit)
                await db.close()

                if entries:
                    msg = "**Historical Matches:**\n\n"
                    for entry in entries[:10]:
                        msg += f"#{entry['id']} {entry.get('title', 'No title')} ({entry.get('type', 'match')})\n"
                    if len(entries) > 10:
            ...
            try:
                ...
                if results:
                    ...
                    for entry in results:
                        msg += f"#{entry['id']} {entry.get('title', 'No title')}\n"
                    await ctx.send(msg)
                else:
        ...
        async def hof_command(ctx, limit: int = 10):
            try:
                ...
                db = DatabaseManager(self.db_path)
                await db.connect()
                entries = await db.list_entries("legendary", limit=limit)
                await db.close()

                if entries:
                    msg = "**Legendary Matches:**\n\n"
                    for entry in entries:
                        msg += f"#{entry['id']} {entry.get('title', 'No title')}\n"
                    await ctx.send(msg)
                else:
                    await ctx.send("No legendary matches found")
            except Exception as err:
...
async def main():
    ...
    bot = BaseballHistoricalMatchAgentDiscordBot(token=token, prefix=prefix)
    await bot.start()
