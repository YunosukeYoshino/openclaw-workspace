#!/usr/bin/env python3
"""
erotic-nlp-analysis-agent - Discord Botモジュール
"""
...
class EroticNlpAnalysisAgentBot(commands.Bot):
    """erotic-nlp-analysis-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="えっちNLP分析エージェント。自然言語処理によるコンテンツ分析。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="erotic-nlp-analysis-agent",
            description="えっちNLP分析エージェント。自然言語処理によるコンテンツ分析。",
            color=0x00ff00
        )
...
class EroticNlpAnalysisAgentCommands(commands.Cog):
    """erotic-nlp-analysis-agent コマンド"""

    def __init__(self, bot: EroticNlpAnalysisAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> EroticNlpAnalysisAgentBot:
    """Botインスタンス作成"""
    bot = EroticNlpAnalysisAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(EroticNlpAnalysisAgentCommands(bot))
