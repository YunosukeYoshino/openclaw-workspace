...
デプロイエージェント。デプロイメントの管理。

Deploy agent. Manage deployments.
...
```python
from agent import DeployAgentAgent

agent = DeployAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from deploy-agent")
```
