...
Terraformマネージャーエージェント。インフラ管理の自動化。

Terraform manager agent. Automate infrastructure management.
...
```python
from agent import TerraformManagerAgentAgent

agent = TerraformManagerAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from terraform-manager-agent")
```
