...
えっち画像分析エージェント。画像の詳細分析。

Erotic image analyzer agent. Analyze images in detail.
...
```python
from agent import EroticImageAnalyzerAgentAgent

agent = EroticImageAnalyzerAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from erotic-image-analyzer-agent")
```
