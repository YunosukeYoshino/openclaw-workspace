...
APIゲートウェイマネージャーエージェント。APIゲートウェイの管理。

API gateway manager agent. Manage API gateways.
...
```python
from agent import ApiGatewayManagerAgentAgent

agent = ApiGatewayManagerAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from api-gateway-manager-agent")
```
