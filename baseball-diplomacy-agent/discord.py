#!/usr/bin/env python3
"""
baseball-diplomacy-agent - Discord Botモジュール
"""
...
class BaseballDiplomacyAgentBot(commands.Bot):
    """baseball-diplomacy-agent Discord Bot"""
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="baseball-diplomacy-agent",
            description="野球外交エージェント。野球を通じた外交活動。",
            color=0x00ff00
        )
...
class BaseballDiplomacyAgentCommands(commands.Cog):
    """baseball-diplomacy-agent コマンド"""

    def __init__(self, bot: BaseballDiplomacyAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> BaseballDiplomacyAgentBot:
    """Botインスタンス作成"""
    bot = BaseballDiplomacyAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(BaseballDiplomacyAgentCommands(bot))
