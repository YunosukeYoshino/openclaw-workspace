...
野球プロダクションエージェント。制作業務の管理。

Baseball production agent. Manage production operations.
...
```python
from agent import BaseballProductionAgentAgent

agent = BaseballProductionAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from baseball-production-agent")
```
