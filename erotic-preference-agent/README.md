...
えっちプレファレンスエージェント。好みの学習。

Erotic preference agent. Learn user preferences.
...
```python
from agent import EroticPreferenceAgentAgent

agent = EroticPreferenceAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from erotic-preference-agent")
```
