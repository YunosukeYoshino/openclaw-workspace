#!/usr/bin/env python3
"""
iot-data-collector-agent - Discord Botモジュール
"""
...
class IotDataCollectorAgentBot(commands.Bot):
    """iot-data-collector-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="IoTデータコレクターエージェント。IoTデータの収集・集約。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="iot-data-collector-agent",
            description="IoTデータコレクターエージェント。IoTデータの収集・集約。",
            color=0x00ff00
        )
...
class IotDataCollectorAgentCommands(commands.Cog):
    """iot-data-collector-agent コマンド"""

    def __init__(self, bot: IotDataCollectorAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> IotDataCollectorAgentBot:
    """Botインスタンス作成"""
    bot = IotDataCollectorAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(IotDataCollectorAgentCommands(bot))
