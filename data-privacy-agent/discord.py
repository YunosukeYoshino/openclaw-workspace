#!/usr/bin/env python3
"""
data-privacy-agent - Discord Botモジュール
"""
...
class DataPrivacyAgentBot(commands.Bot):
    """data-privacy-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="データプライバシーエージェント。データプライバシーの保護・管理。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        ...
        embed = discord.Embed(
            ...
            description="データプライバシーエージェント。データプライバシーの保護・管理。",
            color=0x00ff00
        )
...
class DataPrivacyAgentCommands(commands.Cog):
    """data-privacy-agent コマンド"""

    def __init__(self, bot: DataPrivacyAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> DataPrivacyAgentBot:
    """Botインスタンス作成"""
    bot = DataPrivacyAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(DataPrivacyAgentCommands(bot))
