...
ロードバランサーエージェント。ロードバランシングの管理。

Load balancer agent. Manage load balancing.
...
```python
from agent import LoadBalancerAgentAgent

agent = LoadBalancerAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from load-balancer-agent")
```
