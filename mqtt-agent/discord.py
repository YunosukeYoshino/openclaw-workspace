...
class MqttAgentBot(commands.Bot):
    """mqtt-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="MQTTエージェント。MQTTプロトコルの管理・通信。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        ...
        embed = discord.Embed(
            title="mqtt-agent",
            description="MQTTエージェント。MQTTプロトコルの管理・通信。",
            color=0x00ff00
        )
...
class MqttAgentCommands(commands.Cog):
    """mqtt-agent コマンド"""

    def __init__(self, bot: MqttAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> MqttAgentBot:
    """Botインスタンス作成"""
    bot = MqttAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(MqttAgentCommands(bot))
