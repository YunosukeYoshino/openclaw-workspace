...
野球ブルペンマネージャーエージェント。リリーフ投手の管理。

Baseball bullpen manager agent. Manage relief pitchers.
...
```python
from agent import BaseballBullpenManagerAgentAgent

agent = BaseballBullpenManagerAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from baseball-bullpen-manager-agent")
```
