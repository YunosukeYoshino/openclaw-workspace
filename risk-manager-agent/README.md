...
リスクマネージャーエージェント。リスクの管理・評価。

Risk manager agent. Manage and assess risks.
...
```python
from agent import RiskManagerAgentAgent

agent = RiskManagerAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from risk-manager-agent")
```
