#!/usr/bin/env python3
"""
incident-coordinator-agent - Discord Botモジュール
"""
...
class IncidentCoordinatorAgentBot(commands.Bot):
    """incident-coordinator-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="インシデントコーディネータエージェント。インシデント対応の調整。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="incident-coordinator-agent",
            description="インシデントコーディネータエージェント。インシデント対応の調整。",
            color=0x00ff00
        )
...
class IncidentCoordinatorAgentCommands(commands.Cog):
    """incident-coordinator-agent コマンド"""

    def __init__(self, bot: IncidentCoordinatorAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> IncidentCoordinatorAgentBot:
    """Botインスタンス作成"""
    bot = IncidentCoordinatorAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(IncidentCoordinatorAgentCommands(bot))
