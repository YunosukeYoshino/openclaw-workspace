...
# -*- coding: utf-8 -*-
"""
Discordボットモジュール - えっちコンテンツマネージャーエージェント
"""
...
class DiscordBot(commands.Bot):
    ...
    async def cmd_info(self, ctx: commands.Context):
        """エージェント情報を表示"""
        embed = discord.Embed(title="えっちコンテンツマネージャーエージェント", description="えっちコンテンツの管理・整理・カテゴリ化を行うエージェント", color=discord.Color.green())
        embed.add_field(name="カテゴリ", value="えっちコンテンツ管理", inline=False)
        await ctx.send(embed=embed)
