...
テスト自動化エージェント。テストの自動化。

Test automation agent. Automate testing.
...
```python
from agent import TestAutomationAgentAgent

agent = TestAutomationAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from test-automation-agent")
```
