#!/usr/bin/env python3
"""Discord Integration for game-narrative-design-agent"""
...
logger = logging.getLogger('game-narrative-design-agent')


class GameNarrativeDesignAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**game-narrative-design-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for game-narrative-design-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = GameNarrativeDesignAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
