#!/usr/bin/env python3
"""Discord Integration for game-character-arc-agent"""
...
logger = logging.getLogger('game-character-arc-agent')


class GameCharacterArcAgentDiscordBot:
    ...
    def _setup_commands(self):
        ...
        async def help_command(ctx):
            help_text = """
**game-character-arc-agent Commands:**
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discord bot for game-character-arc-agent")
    parser.add_argument("--token", help="Discord bot token")
    parser.add_argument("--prefix", default="!", help="Command prefix")
    args = parser.parse_args()

    bot = GameCharacterArcAgentDiscordBot(token=args.token, prefix=args.prefix)
    if bot.run():
