#!/usr/bin/env python3
"""
baseball-social-media-agent - Discord Botモジュール
"""
...
class BaseballSocialMediaAgentBot(commands.Bot):
    """baseball-social-media-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="野球ソーシャルメディアエージェント。SNS運営・コンテンツ管理。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="baseball-social-media-agent",
            description="野球ソーシャルメディアエージェント。SNS運営・コンテンツ管理。",
            color=0x00ff00
        )
...
class BaseballSocialMediaAgentCommands(commands.Cog):
    """baseball-social-media-agent コマンド"""

    def __init__(self, bot: BaseballSocialMediaAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> BaseballSocialMediaAgentBot:
    """Botインスタンス作成"""
    bot = BaseballSocialMediaAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(BaseballSocialMediaAgentCommands(bot))
