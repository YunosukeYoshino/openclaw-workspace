...
BIダッシュボードエージェント。ビジネスインテリジェンスダッシュボードの作成。

BI dashboard agent. Create business intelligence dashboards.
...
```python
from agent import BiDashboardAgentAgent

agent = BiDashboardAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from bi-dashboard-agent")
```
