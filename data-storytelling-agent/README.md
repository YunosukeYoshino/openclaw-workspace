...
データストーリーテリングエージェント。データをストーリーとして伝える。

Data storytelling agent. Tell stories with data.
...
```python
from agent import DataStorytellingAgentAgent

agent = DataStorytellingAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from data-storytelling-agent")
```
