#!/usr/bin/env python3
"""
game-stream-monetization-agent - Discord Botモジュール
"""
...
class GameStreamMonetizationAgentBot(commands.Bot):
    """game-stream-monetization-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="ゲームストリームマネタイゼーションエージェント。ストリーム収益化の管理。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="game-stream-monetization-agent",
            description="ゲームストリームマネタイゼーションエージェント。ストリーム収益化の管理。",
            color=0x00ff00
        )
...
class GameStreamMonetizationAgentCommands(commands.Cog):
    """game-stream-monetization-agent コマンド"""

    def __init__(self, bot: GameStreamMonetizationAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> GameStreamMonetizationAgentBot:
    """Botインスタンス作成"""
    bot = GameStreamMonetizationAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(GameStreamMonetizationAgentCommands(bot))
