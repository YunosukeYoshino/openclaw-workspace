...
# -*- coding: utf-8 -*-
"""
Discordボットモジュール - 野球オールスターゲームエージェント
"""
...
class DiscordBot(commands.Bot):
    ...
    async def cmd_info(self, ctx: commands.Context):
        embed = discord.Embed(title="野球オールスターゲームエージェント", description="野球オールスターゲームを管理するエージェント", color=discord.Color.green())
        embed.add_field(name="カテゴリ", value="野球リーグ・大会", inline=False)
        await ctx.send(embed=embed)
