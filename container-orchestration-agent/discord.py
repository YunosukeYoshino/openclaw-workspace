#!/usr/bin/env python3
"""
container-orchestration-agent - Discord Botモジュール
"""
...
class ContainerOrchestrationAgentBot(commands.Bot):
    """container-orchestration-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="コンテナオーケストレーションエージェント。コンテナのオーケストレーション。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="container-orchestration-agent",
            description="コンテナオーケストレーションエージェント。コンテナのオーケストレーション。",
            color=0x00ff00
        )
...
class ContainerOrchestrationAgentCommands(commands.Cog):
    """container-orchestration-agent コマンド"""

    def __init__(self, bot: ContainerOrchestrationAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> ContainerOrchestrationAgentBot:
    """Botインスタンス作成"""
    bot = ContainerOrchestrationAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(ContainerOrchestrationAgentCommands(bot))
