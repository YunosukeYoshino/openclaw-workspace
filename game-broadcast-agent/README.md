...
ゲームブロードキャストエージェント。配信の管理。

Game broadcast agent. Manage broadcasts.
...
```python
from agent import GameBroadcastAgentAgent

agent = GameBroadcastAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from game-broadcast-agent")
```
