# baseball-simulation-agent

野球シミュレーションエージェント。試合・シーズンのシミュレーション。

Baseball simulation agent. Simulate games and seasons.
...
```python
from agent import BaseballSimulationAgentAgent

agent = BaseballSimulationAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from baseball-simulation-agent")
```
