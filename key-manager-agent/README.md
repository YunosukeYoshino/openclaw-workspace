...
鍵マネージャーエージェント。暗号鍵の管理。

Key manager agent. Manage encryption keys.
...
```python
from agent import KeyManagerAgentAgent

agent = KeyManagerAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from key-manager-agent")
```
