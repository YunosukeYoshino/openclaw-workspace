...
野球メディア放送エージェント。中継放送の管理・運営。

Baseball media broadcast agent. Manage and operate live broadcasts.
...
```python
from agent import BaseballMediaBroadcastAgentAgent

agent = BaseballMediaBroadcastAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from baseball-media-broadcast-agent")
```
