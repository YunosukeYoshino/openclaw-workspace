#!/usr/bin/env python3
"""
erotic-creator-growth-agent - Discord Botモジュール
"""
...
class EroticCreatorGrowthAgentBot(commands.Bot):
    """erotic-creator-growth-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="えっちクリエイターグロースエージェント。クリエイターの成長支援。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="erotic-creator-growth-agent",
            description="えっちクリエイターグロースエージェント。クリエイターの成長支援。",
            color=0x00ff00
        )
...
class EroticCreatorGrowthAgentCommands(commands.Cog):
    """erotic-creator-growth-agent コマンド"""

    def __init__(self, bot: EroticCreatorGrowthAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> EroticCreatorGrowthAgentBot:
    """Botインスタンス作成"""
    bot = EroticCreatorGrowthAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(EroticCreatorGrowthAgentCommands(bot))
