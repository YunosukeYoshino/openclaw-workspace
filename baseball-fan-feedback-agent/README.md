...
野球ファンフィードバックエージェント。フィードバック収集・分析。

Baseball fan feedback agent. Collect and analyze feedback.
...
```python
from agent import BaseballFanFeedbackAgentAgent

agent = BaseballFanFeedbackAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from baseball-fan-feedback-agent")
```
