...
暗号オペレーションエージェント。暗号操作の管理。

Crypto operations agent. Manage cryptographic operations.
...
```python
from agent import CryptoOperationsAgentAgent

agent = CryptoOperationsAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from crypto-operations-agent")
```
