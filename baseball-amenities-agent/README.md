...
野球アメニティエージェント。アメニティの管理。

Baseball amenities agent. Manage amenities.
...
```python
from agent import BaseballAmenitiesAgentAgent

agent = BaseballAmenitiesAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from baseball-amenities-agent")
```
