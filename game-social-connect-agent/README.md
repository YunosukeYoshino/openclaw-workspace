...
ゲームソーシャルコネクトエージェント。プレイヤー同士のつながり。

Game social connect agent. Connect players socially.
...
```python
from agent import GameSocialConnectAgentAgent

agent = GameSocialConnectAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from game-social-connect-agent")
```
