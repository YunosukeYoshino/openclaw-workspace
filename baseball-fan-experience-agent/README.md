...
野球ファンエクスペリエンスエージェント。ファン体験の向上。

Baseball fan experience agent. Enhance fan experience.
...
```python
from agent import BaseballFanExperienceAgentAgent

agent = BaseballFanExperienceAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from baseball-fan-experience-agent")
```
