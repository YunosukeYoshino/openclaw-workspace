#!/usr/bin/env python3
"""
privacy-policy-agent - Discord Botモジュール
"""
...
class PrivacyPolicyAgentBot(commands.Bot):
    """privacy-policy-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="プライバシーポリシーエージェント。プライバシーポリシーの管理・更新。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        ...
        embed = discord.Embed(
            ...
            description="プライバシーポリシーエージェント。プライバシーポリシーの管理・更新。",
            color=0x00ff00
        )
...
class PrivacyPolicyAgentCommands(commands.Cog):
    """privacy-policy-agent コマンド"""

    def __init__(self, bot: PrivacyPolicyAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> PrivacyPolicyAgentBot:
    """Botインスタンス作成"""
    bot = PrivacyPolicyAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(PrivacyPolicyAgentCommands(bot))
