...
# -*- coding: utf-8 -*-
"""
Discordボットモジュール - 野球試合データ収集エージェント
"""
...
class DiscordBot(commands.Bot):
    ...
    async def cmd_info(self, ctx: commands.Context):
        embed = discord.Embed(title="野球試合データ収集エージェント", description="野球試合データを自動収集・記録するエージェント。スコア、プレー記録など", color=discord.Color.green())
        embed.add_field(name="カテゴリ", value="野球試合データ・統計", inline=False)
        await ctx.send(embed=embed)
