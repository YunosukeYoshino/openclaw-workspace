...
コンプライアンスレポーターエージェント。コンプライアンスレポートの作成。

Compliance reporter agent. Create compliance reports.
...
```python
from agent import ComplianceReporterAgentAgent

agent = ComplianceReporterAgentAgent()
...
await bot.connect()
await bot.send_message("Hello from compliance-reporter-agent")
```
