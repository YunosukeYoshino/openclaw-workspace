#!/usr/bin/env python3
"""
erotic-entity-extraction-agent - Discord Botモジュール
"""
...
class EroticEntityExtractionAgentBot(commands.Bot):
    """erotic-entity-extraction-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="えっちエンティティ抽出エージェント。エンティティの抽出・認識。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="erotic-entity-extraction-agent",
            description="えっちエンティティ抽出エージェント。エンティティの抽出・認識。",
            color=0x00ff00
        )
...
class EroticEntityExtractionAgentCommands(commands.Cog):
    """erotic-entity-extraction-agent コマンド"""

    def __init__(self, bot: EroticEntityExtractionAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> EroticEntityExtractionAgentBot:
    """Botインスタンス作成"""
    bot = EroticEntityExtractionAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(EroticEntityExtractionAgentCommands(bot))
