...
アイデンティティガバナンスエージェント。IDガバナンスの実施。

Identity governance agent. Implement identity governance.
...
```python
from agent import IdentityGovernanceAgentAgent
...
await bot.connect()
await bot.send_message("Hello from identity-governance-agent")
```
