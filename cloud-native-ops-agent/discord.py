#!/usr/bin/env python3
"""
cloud-native-ops-agent - Discord Botモジュール
"""
...
class CloudNativeOpsAgentBot(commands.Bot):
    """cloud-native-ops-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="クラウドネイティブ運用エージェント。クラウドネイティブ運用。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="cloud-native-ops-agent",
            description="クラウドネイティブ運用エージェント。クラウドネイティブ運用。",
            color=0x00ff00
        )
...
class CloudNativeOpsAgentCommands(commands.Cog):
    """cloud-native-ops-agent コマンド"""

    def __init__(self, bot: CloudNativeOpsAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> CloudNativeOpsAgentBot:
    """Botインスタンス作成"""
    bot = CloudNativeOpsAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(CloudNativeOpsAgentCommands(bot))
