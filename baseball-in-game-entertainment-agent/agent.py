...
class BaseballInGameEntertainmentAgent(commands.Bot):
    def __init__(self, token):
        intents = discord.Intents.default()
        intents.message_content = True
        super().__init__(command_prefix="!", intents=intents)
        self.token = token
        self.db_path = "baseball-in-game-entertainment-agent.db"
        self.init_db()
...
if __name__ == "__main__":
    ...
    if not token:
        ...
        exit(1)
    bot = BaseballInGameEntertainmentAgent(token)
    bot.run_bot()
