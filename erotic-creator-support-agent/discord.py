#!/usr/bin/env python3
"""
erotic-creator-support-agent - Discord Botモジュール
"""
...
class EroticCreatorSupportAgentBot(commands.Bot):
    """erotic-creator-support-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="えっちクリエイターサポートエージェント。クリエイターへのサポート・相談。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="erotic-creator-support-agent",
            description="えっちクリエイターサポートエージェント。クリエイターへのサポート・相談。",
            color=0x00ff00
        )
...
class EroticCreatorSupportAgentCommands(commands.Cog):
    """erotic-creator-support-agent コマンド"""

    def __init__(self, bot: EroticCreatorSupportAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> EroticCreatorSupportAgentBot:
    """Botインスタンス作成"""
    bot = EroticCreatorSupportAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(EroticCreatorSupportAgentCommands(bot))
