#!/usr/bin/env python3
"""
iot-device-manager-agent - Discord Botモジュール
"""
...
class IotDeviceManagerAgentBot(commands.Bot):
    """iot-device-manager-agent Discord Bot"""
    ...
    async def on_ready(self):
        ...
        activity = discord.Activity(
            ...
            name="IoTデバイスマネージャーエージェント。IoTデバイスの管理・監視。"
        )
    ...
    async def send_help(self, channel: discord.TextChannel):
        """ヘルプ送信"""
        embed = discord.Embed(
            title="iot-device-manager-agent",
            description="IoTデバイスマネージャーエージェント。IoTデバイスの管理・監視。",
            color=0x00ff00
        )
...
class IotDeviceManagerAgentCommands(commands.Cog):
    """iot-device-manager-agent コマンド"""

    def __init__(self, bot: IotDeviceManagerAgentBot):
        self.bot = bot
...
def create_bot(token: Optional[str] = None, command_prefix: str = "!") -> IotDeviceManagerAgentBot:
    """Botインスタンス作成"""
    bot = IotDeviceManagerAgentBot(command_prefix=command_prefix, token=token)

    # コグを追加
    bot.add_cog(IotDeviceManagerAgentCommands(bot))
